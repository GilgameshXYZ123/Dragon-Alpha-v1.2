...
import imgnet.resnet50.Net.ResNet50;
...
public class train 
{
    ...
    public static void training(int epoch) {
        ResNet50 net = new ResNet50().train().init(eg).println();
        
//        Optimizer opt = alpha.optim.AdamW(net.param_map(), lr, 1e-2f).println();
        Optimizer opt = alpha.optim.Adam(net.param_map(), lr).println();
        
//        alpha.stat.load_zip(net, ResNet50.weight);
//        alpha.stat.load_zip(opt, ResNet50.opt_weight); 
        ...
        alpha.stat.save_zip(net, ResNet50.weight); 
        alpha.stat.save_zip(opt, ResNet50.opt_weight);
    }
