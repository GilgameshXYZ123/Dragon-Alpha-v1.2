...
public class train 
{
    ...
    public static void training(int epoch) {
        SENet net = new SENet().train().init(eg).println();
        
        Optimizer opt = alpha.optim.RAdam(net.param_map(), lr).println();
//        Optimizer opt = alpha.optim.Adam(net.param_map(), lr).println();
        
//        alpha.stat.load_zip(net, SENet.weight);
//        alpha.stat.load_zip(opt, SENet.opt_weight); 
...
//        alpha.stat.save_zip(net, SENet.weight); 
//        alpha.stat.save_zip(opt, SENet.opt_weight);
    }
