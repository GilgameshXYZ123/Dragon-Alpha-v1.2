...
{ 
    ...
    public static void forEach(double[][] x, Function<Double, Double> func) {
        for(int i=0;i<x.length;i++)
        for(int j=0;j<x[i].length;j++) {
            x[i][j] = (double) func.apply(x[i][j]);
        }
    }
    
    public static void forEach(float[][] x, Function<Float, Float> func) {
        for(int i=0;i<x.length;i++)
        for(int j=0;j<x[i].length;j++) {
            x[i][j] = (float) func.apply(x[i][j]);
        }
    }
    ...
    public static double max(double[][] x) {
        ...
        for(j=0;j<x[i].length;j++) if(max<x[i][j]) max=x[i][j];
        return max;
    }
    ...
    public static double min(double[][] x) {
        double min = x[0][0];
        for(int i=0,j;i<x.length;i++)
        for(j=0;j<x[i].length;j++) if(min>x[i][j]) min=x[i][j];
        return min;
    }
    ...
    public static void print_shape(double[][] result) {
        System.out.println(result.length+" * "+result[0].length);
    }
    
    public static void print_shape(float[][] result) {
        System.out.println(result.length+" * "+result[0].length);
    }
 ...
 to the input {@code Matrix b}.
    ...
    public static float[] row_linear(float[][] A, float alpha, float beta) {
        int N = A.length, M = A[0].length;
        float[] V = new float[N];
        for(int i=0; i<N; i++) {
            V[i] = 0.0f;
    ...
    public static float[][] row_linear_quadratic(float[][] A,
            ...
            float alpha2, float beta2, float gamma2) 
    {
        ...
        for(int i=0; i<N; i++) {
            ...
            for(int j = 0; j<M; j++){
                float a = A[i][j];
                V1[i] += alpha1 * a + beta1;
                V2[i] += alpha2 * a*a + beta2 * a + gamma2;
            }
        }
        return new float[][]{ V1, V2 };
    }
    
    public static float[] row_linear2(float[][] A, float[][] B,
            float alpha, float beta, float gamma) 
    {
        ...
        for(int i=0; i<N; i++) {
            V[i] = 0.0f;
            for(int j = 0; j<M; j++) {
                V[i] += alpha*A[i][j] + beta*B[i][j] + gamma;
            }
        }
        return V;
    }
    ...
    public static float[] row_quadratic2(float[][] A, float[][] B,
            float k11, float k12, float k22,
            float k1, float k2, float C)
    {
        int N = A.length, M = A[0].length;
        ...
        for(int i = 0; i < N; i++)
        {
            ...
            for(int j = 0; j < M; j++) {
                float a = A[i][j];
                float b = B[i][j];
                V[i] += k11*a*a + k12*a*b + k22*b*b + k1*a + k2*b+ C;
            }
        }
        return V;
    }
    ...
    public static float[][] row_var(boolean unbiased, float[][] X) {
        float[] mean = row_mean(X);
        float[] sqmean = row_squareMean(X);
        float[] var = new float[mean.length];
        
        float k = 1.0f; int M = X[0].length;
        if(unbiased) k = (float) (M / (M - 1.0));
        for(int i=0; i<var.length; i++) {
            float m = mean[i];
            float sm = sqmean[i];
            var[i] = k * (sm - m*m);
        }
        return new float[][] { var, mean, sqmean };
    }
    
    public static float[][] row_std(boolean unbiased, float[][] X) {
        float[] mean = row_mean(X);
        ...
        for(int i=0; i<std.length; i++) {
            ...
            std[i] = (float) Math.sqrt((sm - m*m) * k);
        }
        return new float[][] { std, mean, squareMean };
    }
    ...
    public static float[][] field_var(boolean unbiased, float[][] X) {
        float[] mean = field_mean(X);
        float[] sqmean = field_squareMean(X);
        float[] var = new float[mean.length];
        
        float k = 1.0f;
        if(unbiased) k = (float) (X.length / (X.length - 1.0));
        for(int i=0; i<var.length; i++) {
            float m = mean[i];
            float sm = sqmean[i];
            var[i] = (sm - m*m) * k;
    ...
    public static float[][] field_std(boolean unbiased, float[][] X) {
        float[] mean = field_mean(X);
            ...
            float alpha2, float beta2, float gamma2)
    {
        ...
        for(int j = 0; j < M; j++) {
            ...
            for(int i=0; i<N; i++) {
                float a = A[i][j];
                V1[j] += alpha1*a + beta1;
                V2[j] += alpha2*a*a + beta2*a + gamma2;
            }
        }
        return new float[][] {V1, V2};
    }
    ...
    public static float[][] center_quadratic2(float[][][] X1, float[][][] X2,
            float k11, float k12, float k22,
            float k1, float k2, float C) 
    {
        int dim0 = X1.length;
        ...
        for(int d2=0; d2<dim2; d2++) {
            ...
            for(int d1=0; d1<dim1; d1++) {
                ...
                float x2 = X2[d0][d1][d2];
                v += k11*x1*x1 + k12*x1*x2 + k22*x2*x2 + k1*x1 + k2*x2 + C;
            }
            V[d0][d2] = v;
        }
        return V;
   ...
   public static void sumOfEachField(float alpha, float[][] A, float beta, float[] V)
     ...
     * @return 
     */
    public static double[][] double_eye(int size) {
     ...
     */
    @Passed
    public static double[][] double_eye(int height, int width) {
    ...
    public static int[][] to_int_mat(double[][] x) {
        ...
        for(int i=0; i<arr.length; i++) {
            arr[i] = new int[x[i].length];
            for(int j=0; j<arr[i].length; j++) arr[i][j] = (int) x[i][j];
        }
        return arr;
    }
    
    public static double[][] to_double_mat(int[][] x) {
        double[][] arr = new double[x.length][];
        for(int i=0; i<arr.length; i++) {
            arr[i] = new double[x[i].length];
            for(int j=0; j<arr[i].length; j++) arr[i][j] = x[i][j];
        }
        return arr;
    }
    
    public static double[][] to_double_mat(float[][] x) {
        double[][] arr = new double[x.length][];
        for(int i=0; i<arr.length; i++) {
            arr[i] = new double[x[i].length];
            for(int j=0; j<arr[i].length; j++) arr[i][j] = x[i][j];
    ...
    public static boolean[][] valueOfBooleanMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_boolean_vector(tks[i]);
        return arr;
    }
    public static byte[][] valueOfByteMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_byte_vector(tks[i]);
        return arr;
    }
    public static short[][] valueOfShortMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_short_vector(tks[i]);
        return arr;
    }
    public static int[][] valueOfIntMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_int_vector(tks[i]);
        return arr;
    }
    public static long[][] valueOfLongMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_long_vector(tks[i]);
        return arr;
    }
    public static float[][] valueOfFloatMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_float_vector(tks[i]);
        return arr;
    }
    public static double[][] valueOfDoubleMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_double_vector(tks[i]);
        return arr;
    }
    ...
    public static Integer[][] valueOfNIntMatrix(String value) throws Exception
    {
        ...
        for(int i=0;i<arr.length;i++)
            arr[i]=Vector.to_Integer_vector(tks[i]);
        return arr;
    }
