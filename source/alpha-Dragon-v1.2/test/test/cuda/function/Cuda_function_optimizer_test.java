...
public class Cuda_function_optimizer_test 
{
	...
	int lengthv = height * stride;
        ...
        float[] W = Vector.random_float_vector(length, 0, 1f);
        float[] V = Vector.random_float_vector(length, 0, 1f);
        float[] S = Vector.random_float_vector(length, 0, 1f);
        float[] G = Vector.random_float_vector(length, 0, 1f);
        float[] deltaW = Vector.random_float_vector(length, -1f, 1f);
        ...
        Vector.println("V = ", V, 0, 10);
        Vector.println("deltaW = ", deltaW, 0, 10);
...
//        Vector.Adam(W, deltaW, V, a1, a2, S, b1, b2, 1e-8f, lr_t, length);    
        Vector.Adamax(W, deltaW, V, a1, a2, S, b1, 1e-8f, lr_t, length);
...
//        tW = eg.adam_decay(tW, tV, a1, a2, tS, b1, b2, 1e-8f, tdeltaW, lr_t, 0, 0).c();
        tW = eg.adamax(tW, tV, a1, a2, tS, b1, 1e-8f, tdeltaW, lr_t);
...
//        float[] G2 = eg.valueOf(tG);
        ...
        float spW = Vector.samePercent_relative(W, W2, 1e-4f); System.out.println("spW:" + spW);
        float spV = Vector.samePercent_absolute(V, V2); System.out.println("spV:" + spV);
        float spS = Vector.samePercent_absolute(S, S2); System.out.println("spS:" + spS);
//        float spG = Vector.samePercent_absolute(G, G2); System.out.println("spG:" + spS);
...
//        if(spG < 0.99) throw new RuntimeException();
	...
	int lengthv = height * stride;
        ...
        float[] V = Vector.random_float_vector(length, 0, 1f);
        float[] S = Vector.random_float_vector(length, 0, 1f);
        float[] deltaW = Vector.random_float_vector(length, -1, 1);
