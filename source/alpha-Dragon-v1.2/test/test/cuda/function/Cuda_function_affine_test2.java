...
public class Cuda_function_affine_test2 
{
	...
	int lengthv = height * 2 * 3 * stride;
        ...
        float[] X_mean = Vector.random_float_vector(height, 0, 1f);
        float[] X_square_mean = Vector.random_float_vector(height, 1, 2f);
...
//        Tensor tY = eg.layerNorm(false, tX, tX_mean, tX_square_mean, eps, tA, tB).c();
        ...
        Vector.println(value, 0, 10);

        Tensor tdeltaA1 = eg.affine_deltaA_v1(tdeltaY, tY, tA, tB).c();
        Tensor tdeltaA2 = eg.affine_deltaA_v2(tdeltaY, tX, 2*3*width);
...
//        Tensor[] delta2 = eg.layerNorm_deltaAB_v2(tdeltaY, tX, tX_mean, tX_square_mean, eps);
        ...
        float spA1 = Vector.samePercent_relative(deltaA1, deltaA2, 1e-2f);
        float spA2 = Vector.samePercent_relative(deltaA1, deltaA3, 1e-2f);
        float spA3 = Vector.samePercent_relative(deltaA1, deltaA4, 1e-2f);
