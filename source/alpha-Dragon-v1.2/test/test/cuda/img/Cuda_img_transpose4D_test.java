 ...
 * @author Gilgamesh
 */
public class Cuda_img_transpose4D_test 
{
            ...
            int dimIdx1, int dimIdx2) 
    {
        ...
        byte[] A = Vector.random_byte_vector(dim0 * dim1 * dim2 * dim3);
        
        //CPU-------------------------------------------------------------------
        byte[][][][] mA = Vector.to4D(A, dim0, dim1, dim2, dim3);
        byte[][][][] mB = Vector.img_transpose4D(mA, dimIdx1, dimIdx2);
        byte[] B1 = Vector.flatten(mB);
        
        System.out.print("CPU: "); Vector.println(B1, 0, 10);
        
        //GPU-------------------------------------------------------------------
        Tensor tA = eg.tensor_int8(A, dim0, dim1, dim2, dim3).c();
