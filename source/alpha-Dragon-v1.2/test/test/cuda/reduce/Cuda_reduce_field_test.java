...
public class Cuda_reduce_field_test 
{
    ...
    static Engine eg = alpha.engine.cuda_float32(0, alpha.engine.memp1(alpha.MEM_1GB * 6), alpha.MEM_1GB*4);
	...
	int lengthv = height * 2 * 3 * stride;
        ...
        float[] X2 = Vector.random_float_vector(length, -2f, 3f);
...
//        Matrix.field_quadratic2(mX, mX2, 0, 1, 0, 0, 0, 0, Y1);
...
//        Tensor tY = eg.field_mulSum(tX, tX2, 2 * 3 * width);
...
//        Tensor tY = eg.reduce_field_min(tX, 2 * 3 * width).c();
        ...
        float sp0 = Vector.samePercent_relative(Y1, Y2, 1e-3f); System.out.println("sp0:" + sp0);
	...
	int lengthv = height * stride;
        ...
        float time = (float) timer.record().timeStamp_dif_millis() / nIter;
...
//            testCorrect(256*32*32, 16);
//            testCorrect(256*32*32, 32);
            
//            testCorrect(256*64*64, 16);
//            testCorrect(256*64*64, 32);
            
            testCorrect(128*128*128, 16);
            testCorrect(128*128*128, 32);
            
//            testSpeed(256*16*16, 128);//240.384613 GB/s
        }
        catch(Exception e)
        {
            e.printStackTrace();
