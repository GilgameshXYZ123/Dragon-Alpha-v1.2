...
import cifar10.TNet.Net.TNet;
import static cifar10.TNet.Net.eg;
...
public class train 
{
    ...
    public static void training(int epoch) {
        TNet net = new TNet().train().init(eg).println();
    
        
        Optimizer opt = alpha.optim.Adam(net.param_map(), lr).println();
//        Optimizer opt = alpha.optim.AdamW(net.params(), lr, 1e-2f).println();//30 epoch
