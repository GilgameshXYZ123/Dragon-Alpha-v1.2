...
public class train  {
    ...
    public static void training(int epoch) {
        Net.GatedCNN net = new Net.GatedCNN().train().init(eg).println(); 
...
//        Optimizer opt = alpha.optim.RAdam(net.params()).L2coef(1e-2f);//60 epoch, 345.753, 0.7619, 0.8874
        ...
        Optimizer opt = alpha.optim.AdamW(net.params(), lr, 1e-1f)
                .amsgrad(true)
...
//        Optimizer opt = alpha.optim.SGDMN(net.params(), lr).momentum(0.9f).nestorv(1).L2coef(1e-2f);//50 eooch, 286.068, 0.5828, 1.0f
...
//        alpha.stat.load_zip(opt, opt_weight); 
        ...
        for(int i=0; i<epoch; i++) { 
            System.out.println("\n\nepoch = " + i);
            for(iter.shuffle_sort().reset(); iter.hasNext(); batchIdx++) {
