 ...
 * @author Gilgamesh
 */
public class Winograd2D_f33x22_v2 
{
            ...
            int ph, int pw)//sh = sw = 1
    {
        ...
        for(int ow=0; ow<OW; ow += 3)
        {
            ...
            for(int ic=0; ic<IC; ic++) 
            {
                ...
                float[] h = new float[16];//4*4
                h[0] = x[0] - x[ 8]; h[4] = x[4] + x[ 8]; h[ 8] = x[ 8] - x[4]; h[12] = x[12] - x[4];
                h[1] = x[1] - x[ 9]; h[5] = x[5] + x[ 9]; h[ 9] = x[ 9] - x[5]; h[13] = x[13] - x[5];
                h[2] = x[2] - x[10]; h[6] = x[6] + x[10]; h[10] = x[10] - x[6]; h[14] = x[14] - x[6];
                h[3] = x[3] - x[11]; h[7] = x[7] + x[11]; h[11] = x[11] - x[7]; h[15] = x[15] - x[7];
