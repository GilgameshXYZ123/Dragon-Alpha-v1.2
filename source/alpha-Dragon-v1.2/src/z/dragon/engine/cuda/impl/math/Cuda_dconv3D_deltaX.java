...
public final class Cuda_dconv3D_deltaX 
{
    ...
    public static final int GEMMV2_nblock(int IH, int IW, int N, int IC) {
        int GN = IC, GM = N;
        int bn = 0; for(;;) {
            if(GN > 127) bn += GN >> 7; GN &= 127; if(GN == 0) break;//2^7
            if(GN >  63) bn += 1; GN &=  63; if(GN == 0) break;//2^6
    ...
    public static final int max_streamsize_s1 = 10;
    public static final int max_streamsize_V2 = 10;
    public static final int max_streamsize_KerelSplit = 10;
    public static final int max_streamsize_CrossAdd = 7;
    
    public static final int nstream_s1_SGM(int IH, int IW, int N, int IC, int GM_part) {
        int size = Cuda_dconv3D_deltaX.GEMM_nstream_s1(IH, IW, N, IC) * GM_part;
        return (size < max_streamsize_s1 ? size : max_streamsize_s1);
    }
    //<editor-fold defaultstate="collapsed" desc="GEMM_nstream_s1">
    public static final int GEMM_nstream_s1(int IH, int IW, int N, int IC) {
        int GN = IC, GM = N * IH * IW;
        ...
        int size = sn * sm;//the max stream size is 10
        return (size < max_streamsize_s1 ? size : max_streamsize_s1);
    }
      
    public static final int GEMM_nstream_s1(int GM, int IC) {//GM = N*IH*IW
        int GN = IC;
        int sn = 0; if((GN & (GN - 1)) == 0) sn = 1; else for(;;) {
            if(GN > 127) sn++; GN &= 127; if(GN == 0) break;
            if(GN >  63) sn++; GN &=  63; if(GN == 0) break;
    ...
    public static final int Im2colWinograd_s8_nstream(int FW, int IH, int IW, int N, int IC) {
        int IC64 = (IC & 63);
        int GM32 = ((N * IH) & 31) * IW;
        int size =  ((IC64 == 0 && GM32 == 0)? 1 : GEMM_nstream_s1(GM32, IC64));
    ...
    public static final int Im2colWinograd_s16_nstream(int FW, int IH, int IW, int N, int IC) {
        int IC31 = (IC & 31);
        int GM32 = ((N * IH) & 31) * IW;
        return ((IC31 == 0 && GM32 == 0)? 1 : GEMM_nstream_s1(GM32, IC31));
    }
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="GEMMV2_nstream_s1">
    public static final int GEMMV2_nstream_s1(int N, int IC) {
        int GN = IC, GM = N;
        int sn = 0; if((GN & (GN - 1)) == 0) sn = 1; else for(;;) {
            if(GN > 127) sn++; GN &= 127; if(GN == 0) break;
            if(GN >  63) sn++; GN &=  63; if(GN == 0) break;
        ...
        int size = sn * sm;//the max stream size is 10
        return (size < max_streamsize_V2 ? size : max_streamsize_V2);
    }
    //</editor-fold>
    
    public static final int nstream_KernelSplit_SGM(int IH, int IW, int N, int IC, int sh, int sw, int GM_part) {
        int size = KernelSplit_nstream(IH, IW, N, IC, sh, sw) * GM_part;
        return (size < max_streamsize_KerelSplit ? size : max_streamsize_KerelSplit);
    }
    //<editor-fold defaultstate="collapsed" desc="KernelSplit_nstream">
    public static final int KernelSplit_nstream(int IH, int IW, int N, int IC, int sh, int sw) {
    ...
    //<editor-fold defaultstate="collapsed" desc="CrossAdd_nstream">
    public static final int CrossAdd_nstream(int OH, int OW, int N, int OC) {
        ...
        int size = sn * sm;//the max stream size is 7
        return (size < max_streamsize_CrossAdd ? size : max_streamsize_CrossAdd);
    }
    //</editor-fold>
    //</editor-fold>
    ...
    public static final int[] GEMM_GM_part_slice(int GN, int GM, int GK) {
        ...
        if(GM < max_GM) return null;// no need to split GM, 4194304 = 64 * 65536
    ...
    public static final int GEMM_GM_slice(int GN, int GM, int GK) {
        int max_GM = (max_grid + 1) << 6;
        if(GM < max_GM) return -1;// no need to split GM, 4194304 = 64 * 65536
    ...
    public static final int CrossAdd_GM_slice(int GN, int GM, int GK) {
        int max_GM = (max_grid + 1) << 5;
        if(GM < max_GM) return -1;// no need to split GM, 4194304 = 64 * 65536
        
        boolean flag = (GN > 127) && ((GK & 7) == 0);
        int bm = (flag ? GM >> 5 : GM >> 4);//block_num along GM axis
        if(bm <= max_grid) return -1;//no need to split GM
        
        int GM_slice = grid_slice << (flag ? 5 : 4);
        return GM_slice;
    }
    ...
    public static native void dconv3D_deltaX_s1(long[] streamArray, int length,
            ...
            int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_deltaX_s1_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
    ...
    public static final void DCONV3D_deltaX_s1(long[] streamArray, int length,
            ...
            int ph, int pw)
    {
        ...
        int GM = N * IH * IW;
        int GK = FH * FW * OC;
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_s1(streamArray, length, 
            ...
            dconv3D_deltaX_s1_SGM(streamArray, length,
                    deltaY_address, OH, OW, 
                    W_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC, 
    ...
    public static native void dconv3D_deltaX_s1_texture(long[] streamArray, int length,
            ...
            int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_deltaX_s1_texture_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_s1_texture(long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long W_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int ph, int pw)
    {
        ...
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_s1_texture(streamArray, length,
                    deltaY_address, OH, OW,
                    W_address, FH, FW, 
                    deltaX_address, IH, IW, 
                    N, IC, OC, 
            ...
            dconv3D_deltaX_s1_texture_SGM(streamArray, length, 
                    deltaY_address, OH, OW,
                    W_address, FH, FW,
                    deltaX_address, IH, IW,
                    N, IC, OC,
    ...
    public static native void dconv3D_deltaX_W1(long[] streamArray, int length,
            ...
            int N, int IC, int OC);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_deltaX_W1_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH = FW =1
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_W1(long[] streamArray, int length,
            long deltaY_address, 
            long W_address, 
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC)
    {
        ...
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_W1(streamArray, length,
                    deltaY_address, 
                    W_address, 
                    deltaX_address, IH, IW, 
                    N, IC, OC);
            ...
            dconv3D_deltaX_W1_SGM(streamArray, length, 
                    deltaY_address, 
                    W_address, 
                    deltaX_address, IH, IW,
                    N, IC, OC,
    ...
    public static native void dconv3D_deltaX_kernelSplit(long[] streamArray, int length,
            ...
            int sh, int sw, int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_kernelSplit_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
     ...
     */
    @Passed
    public static native void dconv3D_deltaX_kernelSplit_SGM(long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dCW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_kernelSplit(long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long CW_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int sh, int sw, int ph, int pw)
    {
        ...
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0 ){
            dconv3D_deltaX_kernelSplit(streamArray, length, 
                    deltaY_address, OH, OW,
                    CW_address, FH, FW,
                    deltaX_address, IH, IW,
                    N, IC, OC, 
            ...
            dconv3D_deltaX_kernelSplit_SGM(streamArray, length,
                    deltaY_address, OH, OW,
                    CW_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC,
    ...
    public static native void dconv3D_deltaX_ksImsR(long[] streamArray, int length,
            ...
            int sh, int sw, int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_kernelSplit_ImsR_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
     ...
     */
    @Passed
    public static native void dconv3D_deltaX_ksImsR_SGM(long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dCW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int sh, int sw, int ph, int pw, int GM_slice);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_ksImsR(long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long CW_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    {
        int GN = IC;
        int GM = N * (IH / sh) * (IW / sw);//(IH, IW) % (sh, sw) == 0
        int GK = OC;//GK % OC == 0
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_ksImsR(streamArray, length,
                    deltaY_address, OH, OW,
                    CW_address, FH, FW,
                    deltaX_address, IH, IW, 
                    N, IC, OC, 
            ...
            dconv3D_deltaX_ksImsR_SGM(streamArray, length,
                    deltaY_address, OH, OW,
                    CW_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC, 
    ...
    public static native void dconv3D_deltaX_ksImsR_texture(long[] streamArray, int length,
            ...
            int sh, int sw, int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_kernelSplit_ImsR_texture_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
     ...
     */
    @Passed
    public static native void dconv3D_deltaX_ksImsR_texture_SGM(long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dCW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_ksImsR_texture(long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long CW_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int sh, int sw, int ph, int pw)
    {
        ...
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_ksImsR_texture(streamArray, length,
                    deltaY_address, OH, OW, 
                    CW_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC, 
            ...
            dconv3D_deltaX_ksImsR_texture_SGM(streamArray, length,
                    deltaY_address, OH, OW,
                    CW_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC, 
    ...
    public static native void dconv3D_deltaX_ksIms2R(long[] streamArray, int length,
            ...
            int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_kernelSplit_Ims2R_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
     ...
     */
    @Passed
    public static native void dconv3D_deltaX_ksIms2R_SGM(long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dCW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_ksIms2R(long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long CW_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    {
        int GN = IC;
        int GM = N * (IH >> 1) * (IW >> 1);//(IH, IW) % 2 == 0
        int GK = OC;//GK % OC == 0
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_ksIms2R(streamArray, length,
                    deltaY_address, OH, OW,
                    CW_address, FH, FW,
                    deltaX_address, IH, IW,
                    N, IC, OC, 
            ...
            dconv3D_deltaX_ksIms2R_SGM(streamArray, length,
                    deltaY_address, OH, OW, 
                    CW_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC,
    ...
    public static native void dconv3D_deltaX_ksIms2R_texture(long[] streamArray, int length,
            ...
            int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_kernelSplit_Ims2R_texture_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
     ...
     */
    @Passed
    public static native void dconv3D_deltaX_ksIms2R_texture_SGM(long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dCW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_ksIms2R_texture(long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long CW_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int ph, int pw)
    {
        ...
        int GM_slice = GEMM_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_ksIms2R_texture(streamArray, length,
                    deltaY_address, OH, OW,
                    CW_address, FH, FW,
                    deltaX_address, IH, IW,
                    N, IC, OC, 
            ...
            dconv3D_deltaX_ksIms2R_texture_SGM(streamArray, length,
                    deltaY_address, OH, OW, 
                    CW_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC,
    ...
    public static native void dconv3D_deltaX_crossAdd(long[] streamArray, int length,
            ...
            int sh, int sw, int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_crossAdd_SGM">
    /**
     * <pre>
     * deltaX[N, IH, IW, IC] = dconv3D(deltaY[N, OH, OW, OC], W[OC, FH, FW, IC], [sh, sw], [ph, pw]).
     * (1) FH * FW >= 2
     ...
     */
    @Passed
    public static native void dconv3D_deltaX_crossAdd_SGM(long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final void DCONV3D_deltaX_crossAdd(long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long W_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int sh, int sw, int ph, int pw)
    {
        ...
        int GM = N * OH * OW;
        int GK = FH * FW * IC;    
        int GM_slice = CrossAdd_GM_slice(GN, GM, GK);
        if(GM_slice < 0) {
            dconv3D_deltaX_crossAdd(streamArray, length, 
                    deltaY_address, OH, OW, 
                    W_address, FH, FW, 
                    deltaX_address, IH, IW, 
                    N, IC, OC,
            ...
            dconv3D_deltaX_crossAdd_SGM(streamArray, length,
                    deltaY_address, OH, OW, 
                    W_address, FH, FW,
                    deltaX_address, IH, IW,
                    N, IC, OC, 
    ...
    //======[Im2col Winograd for sh = sw = 1]===================================
    //<editor-fold defaultstate="collapsed" desc="Im2col_Winograd_s8_texture">
    /**
     * <pre>
     ...
     */
    @Passed
    public static native boolean Im2col_Winograd_s8_texture(boolean useTexture, long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="Im2col_Winograd_s8_texture_SGM">
    /**
     * <pre>
     * Im2col-Winograd(n, r): n + r = 8 + 1.
     * </pre>
     ...
     */
    @Passed
    public static native boolean Im2col_Winograd_s8_texture_SGM(boolean useTexture, long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final boolean DCONV3D_deltaX_Im2col_Winograd_s8_texture(boolean useTexture, long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long W_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int ph, int pw)
    {
        ...
        if(GM_slice < 0) {
            return Im2col_Winograd_s8_texture(useTexture, streamArray, length,
                    deltaY_address, OH, OW,
                    W_address, FH, FW, 
                    deltaX_address, IH, IW, 
                    N, IC, OC,
            ...
            return Im2col_Winograd_s8_texture_SGM(useTexture, streamArray, length, 
                    deltaY_address, OH, OW, 
                    W_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC, 
    ...
    //<editor-fold defaultstate="collapsed" desc="Im2col_Winograd_s16_texture">
    /**
     * <pre>
     ...
     */
    @Passed
    protected static native boolean Im2col_Winograd_s16_texture(boolean useTexture, long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int ph, int pw);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="Im2col_Winograd_s16_texture_SGM">
    /**
     * <pre>
     * Remode: W[OC, FH, FW, IC] -> CW[FH, FW, IC, OC]
     * Im2col-Winograd(n, r): n + r = 16 + 1.
     ...
     */
    @Passed
    protected static native boolean Im2col_Winograd_s16_texture_SGM(boolean useTexture, long[] streamArray, int length,
            long d_deltaY_address, int OH, int OW,
            long dW_address, int FH, int FW,
            long d_deltaX_address, int IH, int IW,
            int N, int IC, int OC,
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="integration">
    public static final boolean DCONV3D_deltaX_Im2col_Winograd_s16_texture(boolean useTexture, long[] streamArray, int length,
            long deltaY_address, int OH, int OW,
            long W_address, int FH, int FW,
            long deltaX_address, int IH, int IW,
            int N, int IC, int OC,
            int ph, int pw)
    {
        ...
        if(GM_slice < 0) {
            return Im2col_Winograd_s16_texture(useTexture, streamArray, length,
                    deltaY_address, OH, OW,
                    W_address, FH, FW, 
                    deltaX_address, IH, IW, 
                    N, IC, OC,
            ...
            return Im2col_Winograd_s16_texture_SGM(useTexture, streamArray, length, 
                    deltaY_address, OH, OW, 
                    W_address, FH, FW, 
                    deltaX_address, IH, IW,
                    N, IC, OC, 
