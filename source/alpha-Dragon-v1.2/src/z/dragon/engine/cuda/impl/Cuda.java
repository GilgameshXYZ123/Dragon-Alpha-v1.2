...
public final class Cuda {
    ...
    public static String nvidia_smi() throws IOException {
        Process process = Runtime.getRuntime().exec("nvidia-smi");
        InputStream in = process.getInputStream();

        StringBuilder sb = new StringBuilder(4096);
        byte[] buffer = new byte[2048]; int len;
        while((len = in.read(buffer)) != -1) sb.append(new String(buffer, 0, len)); 
        return sb.toString();
    }
    
    public static void nvidia_smi_view() { nvidia_smi_view("nvidia-smi", 2, false); }
    public static void nvidia_smi_view(String title, int fps, boolean daemon) {
        int height = 700, width = 825;
        ...
        JFrame frame = new JFrame(title); frame.add(panel);
        ...
        Thread t = new Thread(()->{
            ...
            while(frame.isVisible()) {
                try {  label.setText(Cuda.nvidia_smi()); } 
    ...
    public static native void streamAddCallback(long stream_address, CudaStreamCallback callback);
    //</editor-fold>
    ...
    public static native void eventSync_Del(long event_address);
    ...
    //<editor-fold defaultstate="collapsed" desc="Low Level Operators of Cuda float(float32)">
    ...
    public static native void set1D_char(long stream_address, long address, int value, int length);
    public static native void set2D_char(long stream_address, long address, int value, int height, int width, int stride);
    ...
    public static native void get2D_v2_char_W3S4(long stream_address, 
