...
public final class Cuda_dconv3D_deltaW {
    ...
    public static final float psu(int IH, int IW, int OH, int OW, int FH, int FW, int sh, int sw) {
        int input_size = IH * IW;
        int padding_size = ((OH - 1)*sh + FH) * ((OW - 1)*sw + FW);
        return 1.0f * padding_size / input_size;
    }
    
    public static final float psu_s1(int IH, int IW, int OH, int OW, int FH, int FW) {
        int input_size = IH * IW;
        int padding_size = ((OH - 1) + FH) * ((OW - 1) + FW);
        return 1.0f * padding_size / input_size;
    }
    ...
    public static final int GEMMV2_nblock(int FH, int FW, int OC, int IC) {
        int GN = OC, GM = IC;
        int bn = 0; for(;;) {
            if(GN > 127) bn += GN >> 7; GN &= 127; if(GN == 0) break;//2^7
            if(GN >  63) bn += 1;       GN &=  63; if(GN == 0) break;//2^6
    ...
    public static final int GEMM_nstream(int FH, int FW, int OC, int IC) {
        ...
        int size = sn * sm;
        return (size < max_streamsize ? size : max_streamsize);
    }

    public static final int GEMMV2_nstream(int OC, int IC) {
        int GN = OC, GM = IC;
        int sn = 0; if ((GN & (GN - 1))== 0) sn = 1; else for(;;) {
            if (GN > 127) sn++; GN &= 127; if(GN == 0) break;
            if (GN >  63) sn++; GN &=  63; if(GN == 0) break;
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="Im2colWinogradV2_nstream">
    public static final int Im2colWinogradV2_s4_nstream(int OC, int IC, int OW, int r) { 
        int size = Im2colWinogradV2_64x64_nstream(OC, IC); 
        if ((size < 2) && (OW % r != 0)) size++;
        return size;
    }
    public static final int Im2colWinogradV2_s8_nstream(int OC, int IC, int OW, int r) { 
        int size = Im2colWinogradV2_64x32_nstream(OC, IC);
        if ((size < 2) && (OW % r != 0)) size++;
        return size;
    }
    public static final int Im2colWinogradV2_s16_nstream(int OC, int IC, int OW, int r, int c64) {
        int size = (c64 != 0 ? 
                Im2colWinogradV2_64x32_nstream(OC, IC) : 
                Im2colWinogradV2_32x32_nstream(OC, IC));
        if ((size < 2) && (OW % r != 0)) size++;
        return size;
    }
    
    static final int Im2colWinogradV2_64x64_nstream(int OC, int IC) {
        int GN = OC, GM = IC;
        int sn = 0; if((GN & (GN - 1)) ==0) sn = 1; else for(;;) {//OC % 64
            if (GN > 63) sn++; GN &= 63; if(GN == 0) break;
            if (GN > 31) sn++; GN &= 31; if(GN == 0) break;
            if (GN > 15) sn++; GN &= 15; if(GN == 0) break;
            if (GN >  7) sn++; GN &=  7; if(GN == 0) break;
        ...
        int sm = 0; if ((GM & (GM - 1)) == 0) sm = 1; else for(;;) {//IC % 64
            if (GM > 63) sm++; GM &= 63; if(GM == 0) break;
            if (GM > 31) sm++; GM &= 31; if(GM == 0) break;
            if (GM > 15) sm++; GM &= 15; if(GM == 0) break;
            if (GM >  7) sm++; GM &=  7; if(GM == 0) break;
    ...
    static final int Im2colWinogradV2_64x32_nstream(int OC, int IC) {
        int GN = OC, GM = IC;
        int sn = 0; if ((GN & (GN - 1)) == 0) sn = 1; else for(;;) {//OC % 64
            if (GN > 63) sn++; GN &= 63; if(GN == 0) break;
            if (GN > 31) sn++; GN &= 31; if(GN == 0) break;
        ...
        int sm = 0; if ((GM & (GM - 1)) == 0) sm = 1; else for(;;) {//IC % 32
            if (GM > 31) sm++; GM &= 31; if(GM == 0) break;
            if (GM > 15) sm++; GM &= 15; if(GM == 0) break;
            if (GM >  7) sm++; GM &=  7; if(GM == 0) break;
            if (GM >  3) sm++; break;
    ...
    static final int Im2colWinogradV2_32x32_nstream(int OC, int IC) {
        int GN = OC, GM = IC;
        int sn = 0; if((GN & (GN - 1)) == 0) sn = 1; else for(;;) {//OC % 32
            if(GN > 31) sn++; GN &= 31; if(GN == 0) break;
            if(GN > 15) sn++; GN &= 15; if(GN == 0) break;
            if(GN >  7) sn++; GN &=  7; if(GN == 0) break;
            if(GN >  3) sn++; break;
    ...
    //======[Im2colWinogradV2 Split H&W Area]===================================
    //<editor-fold defaultstate="collapsed" desc="dconv3D_deltaW_Im2col_WinogradV2_SHW_s4_texture">
    /**
     * <pre>
     * Split OH x OW: OH_slice * OW_slice
     * Im2col-Winograd(n, r): n + r = 4 + 1
     * sh = sw = 1.
     * </pre>
     * @param streamArray
     * @param length
     ...
     * @return 
     */
    public static native boolean Im2col_WinogradV2_SHW_s4_texture(long[] streamArray, int length, int GridZ, int OH_slice, int OW_slice,
            long dX_address, int IH, int IW,
            long d_deltaY_address, int OH, int OW,
            long d_deltaW_address, 
            long d_deltaW_buf_address, int FH, int FW, 
            ...
            int ph, int pw, int n);
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_deltaW_Im2col_WinogradV2_SHW_s8_texture">
    /**
     * <pre>
     * Split OH x OW: OH_slice * OW_slice
     ...
     * @return 
     */
    public static native boolean Im2col_WinogradV2_SHW_s8_texture(long[] streamArray, int length, int GridZ, int OH_slice, int OW_slice,
            long dX_address, int IH, int IW,
            long d_deltaY_address, int OH, int OW,
            long d_deltaW_address, 
            long d_deltaW_buf_address, int FH, int FW, 
            int N, int IC, int OC,
            int ph, int pw, int n);//F(n, r)
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="dconv3D_deltaW_Im2col_WinogradV2_SHW_s16_texture">
    /**
     * <pre>
     * Split OH x OW: OH_slice * OW_slice
     ...
     * @return 
     */
    public static native boolean Im2col_WinogradV2_SHW_s16_texture(long[] streamArray, int length, int GridZ, int OH_slice, int OW_slice, int c64,
            long dX_address, int IH, int IW,
            long d_deltaY_address, int OH, int OW,
            long d_deltaW_address, 
            long d_deltaW_buf_address, int FH, int FW, 
