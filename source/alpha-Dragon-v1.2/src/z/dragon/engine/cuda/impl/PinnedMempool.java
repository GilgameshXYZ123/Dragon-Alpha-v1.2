...
public class PinnedMempool implements MemStatus, Serializable
{
    ...
    public PinnedMempool(long maxMemorySize) { this.max_mem_size = maxMemorySize; }
    ...
    public final Addrpool pool(){ return pool; }
    public final ArrayList_int64 gc() { return gc; }
    ...
    public final int physical_block_num() { return gc.size(); }
    public final int buffered_block_num() { return pool.size(); }
    ...
    public void append(StringBuilder sb) {
        ...
        this.meta_data().forEach((String key, Object value)-> {
    ...
    protected void memory_insufficient() {
        throw new RuntimeException(String.format(
                "[Error] Maximum Memory { %d } is insufficient. Try to alloc more",
                max_mem_size_MB()));
    }
    ...
    protected final long[] alloc(final long mem_size) {//alloc new memory
    ...
    public synchronized long[] malloc(long mem_size) {//mem_size = mem_length * sizeof_datatype
        //----------------------------------------------------------------------
        mem_size = (mem_size + 3) >> 2 << 2;//block is produced by alloc: block[0] = mem_size, so mem_size % 4 == 0;
        //----------------------------------------------------------------------
    ...
    protected void __clear__() { pool.clear(); gc.clear(); }
