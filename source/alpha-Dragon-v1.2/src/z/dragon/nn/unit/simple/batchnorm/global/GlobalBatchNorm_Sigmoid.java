 ...
 * Y = sigmoid(global_batchNorm(X, A, B))
 * @author Gilgamesh
 */
@Passed("CudaFloat32Base")
public class GlobalBatchNorm_Sigmoid extends GlobalBatchNorm {
    private static final long serialVersionUID = 1L;
    
    public GlobalBatchNorm_Sigmoid(boolean inplace, boolean affine, 
    ...
    @Override public GlobalBatchNorm_Sigmoid affine(boolean flag) { affine = flag; return this; }
    @Override public GlobalBatchNorm_Sigmoid beta1(float beta1) { this.beta1 = beta1; return this; }
    @Override public GlobalBatchNorm_Sigmoid beta2(float beta2) { this.beta2 = beta2; return this; }
    @Override public GlobalBatchNorm_Sigmoid eps(float eps) { this.eps = eps; return this; }
   
    @Override public GlobalBatchNorm_Sigmoid weight(Tensor weight) { set_weight(weight); return this; }
    @Override public GlobalBatchNorm_Sigmoid bias(Tensor bias) { set_bias(bias); return this; }
      
    @Override public GlobalBatchNorm_Sigmoid run_mean(Tensor mean) { set_run_mean(mean); return this; }
    @Override public GlobalBatchNorm_Sigmoid run_var(Tensor var) { set_run_var(var); return this; }
    
    @Override public GlobalBatchNorm_Sigmoid train() { this.training = true; return this; }
    @Override public GlobalBatchNorm_Sigmoid eval() { this.training = false; return this; }
    
    @Override
    protected InlineGlobalBatchNorm_Sigmoid<?> create_unit_core() {
        return new InlineGlobalBatchNorm_Sigmoid<>(this);
    }
    //</editor-fold>
    
    //<editor-fold defaultstate="collapsed" desc="static class: InlineGlobalBatchNorm_Sigmoid">
    public static class InlineGlobalBatchNorm_Sigmoid
            <T extends GlobalBatchNorm_Sigmoid>extends InlineGlobalBatchNorm<T>
    {
        public InlineGlobalBatchNorm_Sigmoid(T unit) { super(unit); }
        ...
        protected Tensor __forward_evaluate__(Engine eg, Tensor X, boolean inplace) {
            return (ut.affine?
                    eg.batchNorm_sigmoid(inplace, X, ut.run_mean.ts(), ut.run_var.ts(), ut.eps, ut.A.ts(), ut.B.ts()) :
                    eg.batchNorm_sigmoid(inplace, X, ut.run_mean.ts(), ut.run_var.ts(), ut.eps));
        }
        ...
        protected Tensor __backward_no_affine__(Engine eg, Tensor deltaY, 
            ...
            return (is_holdY() ? //V1: Y is not changed / V2: X is not changed
                    eg.batchNorm_sigmoid_deltaX_v1(grad_inplace, deltaY, holdY(), ut.run_var.ts(), ut.eps):
                    eg.batchNorm_sigmoid_deltaX_v2(grad_inplace, deltaY, holdX(), ut.run_mean.ts(), ut.run_var.ts(), ut.eps));
        }
        ...
        protected Tensor __backward__(Engine eg, Tensor deltaY, 
            ...
            if(ut.A.need_grads() || ut.B.need_grads() || backward_grads) {
                Tensor[] grads = (is_holdY() ? //V1: Y is not changed / V2: X is not changed
                        eg.batchNorm_sigmoid_gradients_v1(grad_inplace, deltaY, holdY(), 
                                ut.run_var.ts(), ut.eps, ut.A.ts(), ut.B.ts()) :
                        eg.batchNorm_sigmoid_gradients_v2(grad_inplace, deltaY, holdX(), 
