...
public class Conv2D extends SimpleUnit {
    private static final long serialVersionUID = 562781240110009L;
    ...
    public int[] fans() { return new int[] { FW*IC, FW*OC }; }//[fan_in, fan_out]
    
    public int[] out_size() { return new int[] { OW }; }
    public Conv2D out_size(int... out_size) {
    ...
    public Conv2D weight(Tensor weight) {
        if(Tensor.isNull(weight)) throw new NullPointerException("weight is null");
        if(!weight.dimEquals(OC, FW, IC)) throw new IllegalArgumentException(String.format(
                "%s : weight.dim { got %s } != [OC, FW, IC] = [%d, %d, %d]", 
                name, Arrays.toString(weight.dim()), OC, FW, IC));
        if(W != null) W.delete();
        W.tensor(weight);
        return this;
    }
    ...
    public Conv2D bias(Tensor bias) {
    ...
    public Conv2D pre_alloc_forward(boolean flag) { pre_alloc_forward = flag; return this; }
    ...
    @Override 
    protected InlineConv2D create_unit_core() {
        return new InlineConv2D(this); 
    }
    ...
    protected void __init__(Engine eg) {
        ...
        Tensor tW = eg.empty(OC, FW, IC);//init convolutional kernel W
    ...
    //<editor-fold defaultstate="collapsed" desc="static class: InlineConv2D"> 
    public static class InlineConv2D extends SimpleCore<Conv2D> {
        transient private int IW;
        transient private Tensor y;

        public InlineConv2D(Conv2D unit) { super(unit); }
        ...
        protected void __before_forward__(Engine eg, Tensor X) { 
            ...
            y = (ut.pre_alloc_forward ? //X[N, IW, IC] -> Y[N, OW, OC]
                    eg.alloc.conv2D(X, ut.W.ts(), ut.OW, ut.sw, ut.pw) : 
                    null);
        }
        ...
        protected Tensor __forward__(Engine eg, Tensor X) {
            if(ut.pre_alloc_forward) { 
                ...
                return (ut.biased ? //X[N, IW, IC] -> Y[N, OW, OC]
                    eg.conv2D_biased(out, X, ut.W.ts(), ut.sw, ut.pw, ut.B.ts()) :
                    eg.conv2D(out, X, ut.W.ts(), ut.sw, ut.pw));
            }
            
            return (ut.biased ? //X[N, IW, IC] -> Y[N, OW, OC]
                   eg.conv2D_biased(X, ut.W.ts(), ut.OW, ut.sw, ut.pw, ut.B.ts()) :
                   eg.conv2D(X, ut.W.ts(), ut.OW, ut.sw, ut.pw));
        }
        //</editor-fold>
                ...
                boolean grad_inplace, boolean backward_grads)
        {
            ...
            if(ut.W.need_grads()) {//find the gradient of filters
                deltaW = eg.conv2D_deltaW(holdX(), deltaY, ut.FW, ut.sw, ut.pw);
            ...
            if(backward_grads) {//find the gradient of input-features
                deltaX = eg.conv2D_deltaX(deltaY, ut.W.ts(), IW, ut.sw, ut.pw);
