...
public class Affine_Sigmoid extends Affine {
    private static final long serialVersionUID = 1L;
    
    public Affine_Sigmoid(boolean inplace, int... feature_dim) {
        super(inplace, feature_dim);
    }
    
    //<editor-fold defaultstate="collapsed" desc="functions">
    @Override public Affine_Sigmoid weight(Tensor weight) { set_weight(weight); return this; }
    @Override public Affine_Sigmoid bias(Tensor bias) { set_bias(bias); return this; }
    
    @Override
    protected InlineAffine_Sigmoid<?> create_unit_core() {
        return new InlineAffine_Sigmoid<>(this);
    }
    //</editor-fold>
    
    //<editor-fold defaultstate="collapsed" desc="static class: InlineAffine_Sigmoid">
    public static class InlineAffine_Sigmoid<T extends Affine_Sigmoid> extends InlineAffine<T> 
    {
        public InlineAffine_Sigmoid(T unit) { super(unit); }
        
        //<editor-fold defaultstate="collapsed" desc="running-area: propagation"> 
        @Override
        protected Tensor __forward__(Engine eg, Tensor X, boolean inplace) {
            return eg.affine_sigmoid(inplace, X, ut.A.ts(), ut.B.ts());
        }
        ...
        protected Tensor __backward__(Engine eg, Tensor deltaY, 
            ...
            if(ut.A.need_grads() || ut.B.need_grads()) {//A.need_grads = B.need_grads = true
                Tensor[] grads = (is_holdY()? //V1: Y is not changed / V2: X is not changed
                        eg.affine_sigmoid_deltaAB_v1(deltaY, holdY(), ut.A.ts(), ut.B.ts()) :
                        eg.affine_sigmoid_deltaAB_v2(deltaY, holdX(), ut.A.ts(), ut.B.ts()));
            ...
            if(backward_grads) {
                deltaX = (is_holdY()? //V1: Y is not changed / V2: X is not changed
                        eg.affine_sigmoid_deltaX_v1(false, deltaY, holdY(), ut.A.ts()) :
                        eg.affine_sigmoid_deltaX_v2(false, deltaY, holdX(), ut.A.ts(), ut.B.ts()));
