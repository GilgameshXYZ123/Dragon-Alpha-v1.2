 ...
 * @param <T>
 */
public class CoreMatMulT2 <T extends DualUnit> extends DualCore<T> 
{
    public CoreMatMulT2(T unit) { super(unit); }
            ...
            boolean backward_grads1, boolean backward_grads2) 
    {
        ...
        //(1) deltaX1[N, K] = matMul:   deltaY[N, M]   * X2[M, K]
        //(2) deltaX2[M, K] = matMulT1: deltaY^T[M, N] * X1[N, K]
        if(backward_grads1) { deltaX1 = eg.matMul(deltaY, holdX2()); count++; }
        if(backward_grads2) { deltaX2 = eg.matMulT1(deltaY, holdX1()); count++; }
