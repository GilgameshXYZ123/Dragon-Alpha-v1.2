...
/**
 * Linear2 + Sigmoid.
 * Y = sigmoid(alpha*X1 + beta*X2 + gamma, k)
 * @author Gilgamesh
 * @param <T>
 */
public class CoreLinear2_Sigmoid<T extends DualUnit> extends DualCore<T> {
    protected boolean likeX1;
    protected float alpha, beta, gamma;

    public CoreLinear2_Sigmoid(T unit, boolean likeX1, 
    ...
    protected Tensor __forward__(Engine eg, Tensor X1, Tensor X2) {
        return eg.linear2_sigmoid(false, likeX1, X1, X2, 
                alpha, beta, gamma);
    }
    ...
    protected Tensor[] __backward__(Engine eg, Tensor deltaY, 
        ...
        Tensor[] grads = (is_holdY()? //V1: holdY / V2: hold{X1, X2}
                eg.linear2_sigmoid_deltaX_v1(grad_inplace, deltaY, holdY(), alpha, beta) :
                eg.linear2_sigmoid_deltaX_v2(grad_inplace, deltaY, holdX1(), holdX2(), alpha, beta, gamma));
