...
public class CoreAdaptiveMaxPool2D <T extends SimpleUnit> extends CoreAdaptivePool2D<T> {
    ...
    protected Tensor __forward__(Engine eg, Tensor X) {
        __adaptive__(X);
        if(!training) return eg.pool2D_max(X, FH, FW, OH, OW, sh, sw, 0, 0);

        Tensor[] result = eg.pool2D_max_indexed(X, FH, FW, OH, OW, sh, sw, 0, 0);
    ...
    protected Tensor __backward__(Engine eg, Tensor deltaY, 
        ...
        Tensor deltaX = eg.unpool2D_max_indexed(deltaY, Index, IH, IW, FH, FW, sh, sw, 0, 0);
        return (grad_inplace? 
