 ...
 * @param <T>
 */
public class CoreCrop<T extends SimpleUnit> extends SimpleInplaceCore<T> {
    protected int[] start;
    protected int[] out_dim;
    
    transient protected int[] in_dim;
    
    public CoreCrop(T unit, boolean inplace, int[] start_point, int[] out_dim) {
        super(unit, inplace);
    ...
    protected Tensor __forward__(Engine eg, Tensor X, boolean inplace) {
        in_dim = X.dim();
        return eg.crop(inplace, X, start, out_dim);
    }
    ...
    protected Tensor __backward__(Engine eg, Tensor deltaY, 
        ...
        return (backward_grads?
                eg.expand(grad_inplace, deltaY, start, in_dim) : 
                null);
    }
    //</editor-fold>
}
