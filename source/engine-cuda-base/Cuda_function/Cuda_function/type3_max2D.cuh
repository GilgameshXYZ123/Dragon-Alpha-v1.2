...
//lengthv > lengthv_max
#define max2d_k4_max(stream, alpha, X, beta, vmax, Y, lengthv, width, stride)\
	max2D_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { max2d_k4_max(stream, alpha, X, beta, vmax, Y, lengthv, width, stride); return; }
