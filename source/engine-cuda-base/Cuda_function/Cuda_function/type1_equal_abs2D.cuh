...
//lengthv > lengthv_max
#define equal_abs2d_k4_max(stream, X1, X2, min, max, Y, lengthv, width, stride)\
	equal_abs2D_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { equal_abs2d_k4_max(stream, X1, X2, min, max, Y, lengthv, width, stride); return; }
