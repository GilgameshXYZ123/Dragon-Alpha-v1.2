...
//lengthv > lengthv_max
#define sigmoid_binaryCrossEntropy_2d_k4_max(stream, Y, X, alpha, beta, L, lengthv, width, stride)\
	sigmoid_binaryCrossEntropy_2D_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { sigmoid_binaryCrossEntropy_2d_k4_max(stream, Y, X, alpha, beta, L, lengthv, width, stride); return; }
