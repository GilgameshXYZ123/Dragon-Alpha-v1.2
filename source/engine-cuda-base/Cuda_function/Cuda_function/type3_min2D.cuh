...
//lengthv > lengthv_max
#define min2d_k4_max(stream, alpha, X, beta, vmin, Y, lengthv, width, stride)\
	min2D_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { min2d_k4_max(stream, alpha, X, beta, vmin, Y, lengthv, width, stride); return; }
