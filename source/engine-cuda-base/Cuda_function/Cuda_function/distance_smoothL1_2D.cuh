...
//lengthv > lengthv_max
#define smoothL1_2d_k4_max(stream, Y, Yh, L, lengthv, width, stride)\
	smoothL1_2D_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	for (int index4 = index << 2; index4 < lengthv; index4 += step4)
	{
		...
		loss.x = fabsf(yh.x - y.x); loss.x = (loss.x - 0.5f) + (1.0f > loss.x)*(0.5f*loss.x*loss.x + 0.5f - loss.x);
		loss.y = fabsf(yh.y - y.y); loss.y = (loss.y - 0.5f) + (1.0f > loss.y)*(0.5f*loss.y*loss.y + 0.5f - loss.y);
		loss.z = fabsf(yh.z - y.z); loss.z = (loss.z - 0.5f) + (1.0f > loss.z)*(0.5f*loss.z*loss.z + 0.5f - loss.z);
		loss.w = fabsf(yh.w - y.w); loss.w = (loss.w - 0.5f) + (1.0f > loss.w)*(0.5f*loss.w*loss.w + 0.5f - loss.w);
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { smoothL1_2d_k4_max(stream, Y, Yh, L, lengthv, width, stride); return; }
