...
//lengthv > lengthv_max
#define softplus2d_k4_max(stream, X, Y, lengthv, width, stride)\
	softplus2D_kernel_4\
...
//Y = softPlus(x) = log(1 + e^x) = log1p(exp(x)).
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { softplus2d_k4_max(stream, X, Y, lengthv, width, stride); return; }
