...
//lengthv > lengthv_max
#define log2d_deltaX_k4_max(stream, deltaX, deltaY, Y, alpha, lengthv, width, stride)\
	log2D_deltaX_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { log2d_deltaX_k4_max(stream, deltaX, deltaY, Y, alpha, lengthv, width, stride); return; }
