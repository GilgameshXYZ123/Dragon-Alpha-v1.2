...
//lengthv > lengthv_max
#define sqBatchNorm_affined2d_row_k4_max(stream, X, X_mean, X_sqmean, eps, A, B, row_lengthv, Y, lengthv, width, stride)\
	sqBatchNorm_affined2D_row_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { sqBatchNorm_affined2d_row_k4_max(stream, X, X_mean, X_sqmean, eps, A, B, row_lengthv, Y, lengthv, width, stride); return; }
