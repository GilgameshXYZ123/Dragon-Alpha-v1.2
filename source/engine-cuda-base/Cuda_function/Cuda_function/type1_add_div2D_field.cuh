...
//lengthv > lengthv_max
#define add_div2d_field_k4_max(stream, X1, X2, X3, row_lengthv, alpha, beta, gamma, delta, Y, lengthv, width, stride)\
	add_div2D_field_kernel_4\
...
__global__ void add_div2D_field_kernel_4(
	...
	const float* __restrict__ X3, int row_lengthv,
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { add_div2d_field_k4_max(stream, X1, X2, X3, row_lengthv, alpha, beta, gamma, delta, Y, lengthv, width, stride); return; }
