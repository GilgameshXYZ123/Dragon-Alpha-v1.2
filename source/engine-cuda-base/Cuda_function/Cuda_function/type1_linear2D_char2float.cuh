...
//lengthv > lengthv_max
#define linear2d_char2float_k4_max(stream, alpha, X, beta, Y, lengthv, width, stride)\
	linear2D_char2float_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { linear2d_char2float_k4_max(stream, alpha, X, beta, Y, lengthv, width, stride); return; }
