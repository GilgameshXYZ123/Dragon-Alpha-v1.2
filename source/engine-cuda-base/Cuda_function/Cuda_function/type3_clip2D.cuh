...
//lengthv > lengthv_max
#define clip2d_k4_max(stream, alpha, X, beta, vmin, vmax, Y, lengthv, width, stride)\
	clip2D_kernel_4\
	...
	int lengthv, int width, int stride)
{
	...
	if (lengthv > LENGTHV_MAX) { clip2d_k4_max(stream, alpha, X, beta, vmin, vmax, Y, lengthv, width, stride); return; }
