...
//lengthv > lengthv_max
#define layernorm_affined2d_row_deltaX_v1_k4_max(stream, deltaY, Y, X_field_mean, X_field_square_mean, eps, A, B, deltaXp1, deltaXp2, row_lengthv, deltaX, lengthv, width, stride)\
	layernorm_affined2D_row_deltaX_v1_kernel_4\
...
void __layernorm_affined2D_row_deltaX_v1(cudaStream_t stream,
	...
	const float* X_field_sqmean, float eps,
	const float* A, const float* B,
	const float* deltaXp1, const float* deltaXp2, int row_lengthv,
	      float* deltaX,
	int lengthv, int width, int stride)
{
	if (lengthv < 256) { layernorm_affined2d_row_deltaX_v1_k4_small(stream, deltaY, Y, X_field_mean, X_field_sqmean, eps, A, B, deltaXp1, deltaXp2, row_lengthv, deltaX, lengthv, width, stride); return; }
	if (lengthv > LENGTHV_MAX) { layernorm_affined2d_row_deltaX_v1_k4_max(stream, deltaY, Y, X_field_mean, X_field_sqmean, eps, A, B, deltaXp1, deltaXp2, row_lengthv, deltaX, lengthv, width, stride); return; }
	layernorm_affined2d_row_deltaX_v1_k4(stream, 5, 2, deltaY, Y, X_field_mean, X_field_sqmean, eps, A, B, deltaXp1, deltaXp2, row_lengthv, deltaX, lengthv, width, stride);
}
