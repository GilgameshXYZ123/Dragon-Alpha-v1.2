...
#ifndef DECONV_DX_3D_WINOGRAD_S16_F9X8_H
#define DECONV_DX_3D_WINOGRAD_S16_F9X8_H

//(1) sh = sw = 1
//(2) FW = 8
//(3) IW % 9: group = 9 elements
#ifndef DECONV_DX_3D_WINOGRAD_S16_F9X8_CALL
#define DECONV_DX_3D_WINOGRAD_S16_F9X8_CALL

//<1> (opw <= 3) -> (FW - 1 - pw <= 4) -> (pw >= 3)
//<2> (IW - OW - opw + FW - 1 <= 5) -> (IW - OW + pw <= 5) -> (OW - IW - pw + 5 >= 0)

//================[Standard: 32(IC) * 288(GM)]===============================
//IW % 8 == 0, pw >= 3
#define winograd_f9x8_k32x288_p3_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f9x8_kernel_32x288_p3_tex<FH>\
		<<< dim3(IC>>5, ((N*IH)>>5) * (IW/9)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*8*IC), deltaX,IH,IW, IC,OC, (FH-1-ph),(7-pw))	

//IW % 8 == 0, pw >= 3, template<IC>
#define winograd_f9x8_k32x288_p3_ICT_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f9x8_kernel_32x288_p3_ICT_tex<FH, IC>\
		<<< dim3(IC>>5, ((N*IH)>>5) * (IW/9)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*8*IC), deltaX,IH,IW, OC, (FH-1-ph),(7-pw))	

//IW % 8 == 0, pw >= 3, template<IC, OC>
#define winograd_f9x8_k32x288_p3_CT_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f9x8_kernel_32x288_p3_CT_tex<FH, IC, OC>\
		<<< dim3(IC>>5, ((N*IH)>>5) * (IW/9)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*8*IC), deltaX,IH,IW, (FH-1-ph),(7-pw))	

//IW % 18 == 0, pw >= 3
#define winograd_f9x8_ruse_k32x288_p3_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f9x8_ruse_kernel_32x288_p3_tex<FH>\
		<<< dim3(IC>>5, ((N*IH)>>5)*((IW/18)*2)), dim3(16, 8), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*8*IC), deltaX,IH,IW, IC,OC, (FH-1-ph),(7-pw))

//================[Standard: 64(IC) * 288(GM)]===============================
//IW % 8 == 0, pw >= 3
#define winograd_f9x8_k64x288_p3_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f9x8_kernel_64x288_p3_tex<FH>\
		<<< dim3(IC>>6, ((N*IH)>>5) * (IW/9)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*8*IC), deltaX,IH,IW, IC,OC, (FH-1-ph),(7-pw))	

//IW % 8 == 0, pw >= 3, template<OC>
#define winograd_f9x8_k64x288_p3_OCT_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f9x8_kernel_64x288_p3_OCT_tex<FH, OC>\
		<<< dim3(IC>>6, ((N*IH)>>5) * (IW/9)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*8*IC), deltaX,IH,IW, IC, (FH-1-ph),(7-pw))	

#endif


//================[Standard: 32(IC) * 288(GM)]===============================
//LB = 4, IC % 8 == 0, IW % 8 == 0, pw >= 3
#ifndef DECONV_3D_DX_WINOGRAD_F9X8_32X288_P3_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F9X8_32X288_P3_TEXTURE

//for: Feature = (36, 36), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 81, Time = 8.508 msec, Performace = 20445 GFlop/s
//for: Feature = (18, 18), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 81, Time = 8.105 msec, Performace = 21461.6 GFlop/s

template<int FH>
__global__ void winograd_f9x8_kernel_32x288_p3_tex(
	...
	int oph, int opw)//sh = sw = 1
{
	...
	const int tj0 = bj0 + (Di * 9);//OW % 9
	const int IW9 = (IW / 9) * 9, IH_IW9 = IH * IW9;
	get_n_ih_iw_Temp(tj0, tn0, tih0, tiw0, IH_IW9, IW9);
	...
	const int xj0 = bj0 + (DIdx + (ux >> 1)) * 9;
	get_n_ih_iw_Temp(xj0, xn0, xih0, xiw0, IH_IW9, IW9);
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		const int W7 = W0 + IC * 7;
		w[7] = W[W0]; w[6] = W[W1]; w[5] = W[W2];
		w[4] = W[W3]; w[3] = W[W4]; w[2] = W[W5];
		w[1] = W[W6]; w[0] = W[W7];
		...
		bool lyB = lh0 && (tow0 >= -11) && (tow0 + 11 < OW);//pw = +1
		bool lyC = lh0 && (tow0 >= -12) && (tow0 + 12 < OW);//pw = +2
		bool lyD = lh0 && (tow0 >= -13) && (tow0 + 13 < OW);//pw = +3
		bool lyE = lh0 && (tow0 >= -14) && (tow0 + 14 < OW);//pw = +4
		bool lyF = lh0 && (tow0 >= -15) && (tow0 + 15 < OW);//pw = +5
		int tY0 = Y0 + fh * OW * OC;
		...
		tYA = IF_int(lh0, tYA, -1); 
		tYB = IF_int(lyB, tYB, -1); tYC = IF_int(lyC, tYC, -1); 
		...
		for (int ooc = 8; ooc < OC; ooc += 8) {
			//Winograd Transform: W(8) -> G(16), Y(16) -> D(16)
			winograd_f9x8_G(g, w);
			winograd_f9x8_D(d, x);
...
#pragma unroll
	...
	float a[16], x0[9], x1[9], x2[9], x3[9];//ic0, ic1, ic2, ic3
	__syncthreads();
...
//LB = 4, IC % 8 == 0, IW % 8 == 0, pw >= 3, template<IC>
#ifndef DECONV_3D_DX_WINOGRAD_F9X8_32X288_P3_ICT_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F9X8_32X288_P3_ICT_TEXTURE

//for: Feature = (36, 36), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 81, Time = 7.835 msec, Performace = 22201.2 GFlop/s
//for: Feature = (18, 18), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 81, Time = 7.87  msec, Performace = 22102.4 GFlop/s

template<int FH, int IC>
__global__ void winograd_f9x8_kernel_32x288_p3_ICT_tex(
	cudaTextureObject_t       deltaY, int OH, int OW,
	const float* __restrict__      W,
	      float* __restrict__ deltaX, int IH, int IW,
	int OC,
...
//LB = 4, IC % 8 == 0, IW % 8 == 0, pw >= 3, template<IC, OC>
#ifndef DECONV_3D_DX_WINOGRAD_F9X8_32X288_P3_CT_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F9X8_32X288_P3_CT_TEXTURE

//for: Feature = (36, 36), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 81, Time = 7.515 msec, Performace = 23146.5 GFlop/s
//for: Feature = (18, 18), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 81, Time = 7.33  msec, Performace = 23730.7 GFlop/s

template<int FH, int IC, int OC>
__global__ void winograd_f9x8_kernel_32x288_p3_CT_tex(
	cudaTextureObject_t       deltaY, int OH, int OW,
	const float* __restrict__      W,
	      float* __restrict__ deltaX, int IH, int IW,
	int oph, int opw)//sh = sw = 1
...
//LB = 4, IC % 8 == 0, IW % 16 == 0, pw >= 3
#ifndef DECONV_3D_DX_WINOGRAD_F9X8_RUSE_32X288_P3_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F9X8_RUSE_32X288_P3_TEXTURE

//for: Feature = (72, 72), [N, IC, OC] = [64, 64, 64]
//LB = 4: Size = 81, Time = 5.97 msec, Performace = 29136.7 GFlop/s
//for: Feature = (36, 36), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 81, Time = 6.912 msec, Performace = 25165.8 GFlop/s
//for: Feature = (18, 18), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 81, Time = 6.226 msec, Performace = 27938.7 GFlop/s

template<int FH>
__global__ void winograd_f9x8_ruse_kernel_32x288_p3_tex(
	cudaTextureObject_t       deltaY, int OH, int OW,
	const float* __restrict__      W,
	      float* __restrict__ deltaX, int IH, int IW,
	int IC, int OC,
	int oph, int opw)//sh = sw = 1
{
	...
	//prepare for deltaY[N, OH, OW, IC]
	const int bj0 = by * 288;//32 * 8 = 288
	const int Dk = (tx & 7), Di = ((ty << 1) + (tx > 7)) << 1;//[8, 16*2 = 32]
	const int tj0 = bj0 + (Di * 9);//OW % 9
	const int IW18 = (IW / 18) * 18, IH_IW18 = IH * IW18;//9 * 2 = 18
	get_n_ih_iw_Temp(tj0, tn0, tih0, tiw0, IH_IW18, IW18);
	const int toh0 = tih0 - oph, tow0 = tiw0 - opw;
	const int Y0 = ((tn0*OH + toh0)*OW + tow0)*OC + Dk;//deltaY[tn0, toh0, tow0, Dk]
	...
	const int xic0 = bic0 + GIdx + ((ux & 1) << 2);
	const int xj0 = bj0 + (DIdx + (ux >> 1)) * 9;
	get_n_ih_iw_Temp(xj0, xn0, xih0, xiw0, IH_IW18, IW18);
	const int X00 = ((xn0*IH + xih0)*IW + xiw0)*IC + xic0;
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		const int W5 = W0 + IC * 5, W6 = W0 + IC * 6;
		const int W7 = W0 + IC * 7;
		float2 wv7 = *(float2*)(W + W0); w0[7] = wv7.x; w1[7] = wv7.y;
		float2 wv6 = *(float2*)(W + W1); w0[6] = wv6.x; w1[6] = wv6.y;
		float2 wv5 = *(float2*)(W + W2); w0[5] = wv5.x; w1[5] = wv5.y;
		float2 wv4 = *(float2*)(W + W3); w0[4] = wv4.x; w1[4] = wv4.y;
		float2 wv3 = *(float2*)(W + W4); w0[3] = wv3.x; w1[3] = wv3.y;
		float2 wv2 = *(float2*)(W + W5); w0[2] = wv2.x; w1[2] = wv2.y;
		float2 wv1 = *(float2*)(W + W6); w0[1] = wv1.x; w1[1] = wv1.y;
		float2 wv0 = *(float2*)(W + W7); w0[0] = wv0.x; w1[0] = wv0.y;
		...
		bool ly2 = lh0 && (tow0 >=  -2) && (tow0 +  2 < OW);//pw = -2
		bool ly3 = lh0 && (tow0 >=  -3) && (tow0 +  3 < OW);//pw = -1
		bool lyK = lh0 && (tow0 >= -20) && (tow0 + 20 < OW);//pw = +2
		bool lyL = lh0 && (tow0 >= -21) && (tow0 + 21 < OW);//pw = +3
		bool lyM = lh0 && (tow0 >= -22) && (tow0 + 22 < OW);//pw = +4
		bool lyN = lh0 && (tow0 >= -23) && (tow0 + 23 < OW);//pw = +5
		bool lyO = lh0 && (tow0 >= -24) && (tow0 + 24 < OW);//pw = +5
		...
		tYM = IF_int(lyM, tYM, -1); tYN = IF_int(lyN, tYN, -1); tYO = IF_int(lyO, tYO, -1);
		x[ 0] = tex1Dfetch<float>(deltaY, tY0); x[ 1] = tex1Dfetch<float>(deltaY, tY1);
		x[ 2] = tex1Dfetch<float>(deltaY, tY2); x[ 3] = tex1Dfetch<float>(deltaY, tY3);
		x[ 4] = tex1Dfetch<float>(deltaY, tY4); x[ 5] = tex1Dfetch<float>(deltaY, tY5);
		x[ 6] = tex1Dfetch<float>(deltaY, tY6); x[ 7] = tex1Dfetch<float>(deltaY, tY7);
		...
		x[24] = tex1Dfetch<float>(deltaY, tYO);
...
#ifndef DECONV_3D_DX_WINOGRAD_F9X8_64X288_P3_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F9X8_64X288_P3_TEXTURE

//for: Feature = (72, 72), [N, IC, OC] = [64, 64, 64]
//LB = 4: Size = 162, Time = 10.052 msec, Performace = 34609.3 GFlop/s
//for: Feature = (36, 36), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 162, Time = 12.098 msec, Performace = 28756.2 GFlop/s
//for: Feature = (18, 18), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 162, Time = 11.725 msec, Performace = 29671   GFlop/s

template<int FH>
__global__ void winograd_f9x8_kernel_64x288_p3_tex(
	cudaTextureObject_t       deltaY, int OH, int OW,
	const float* __restrict__      W,
	      float* __restrict__ deltaX, int IH, int IW,
	int IC, int OC,
...
#ifndef DECONV_3D_DX_WINOGRAD_F9X8_64X288_P3_OCT_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F9X8_64X288_P3_OCT_TEXTURE

//for: Feature = (72, 72), [N, IC, OC] = [64, 64, 64]
//LB = 4: Size = 162, Time = 10.052 msec, Performace = 34609.3 GFlop/s
//for: Feature = (36, 36), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 162, Time = 11.967 msec, Performace = 29071   GFlop/s
//for: Feature = (18, 18), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 162, Time = 11.216 msec, Performace = 31017.5 GFlop/s

template<int FH, int OC>
__global__ void winograd_f9x8_kernel_64x288_p3_OCT_tex(
	cudaTextureObject_t       deltaY, int OH, int OW,
	const float* __restrict__      W,
	      float* __restrict__ deltaX, int IH, int IW,
	int IC,
