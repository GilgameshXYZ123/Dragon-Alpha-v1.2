...
#ifndef DECONV_3D_DX_WINOGRAD_S4_F3X2R_H
#define DECONV_3D_DX_WINOGRAD_S4_F3X2R_H

//(1) sh = sw = 1
//(2) FW = 2
//(3) IW % 3: group = 3 elements
#ifndef DECONV_3D_DX_WINOGRAD_S4_F3X2R_CALL
#define DECONV_3D_DX_WINOGRAD_S4_F3X2R_CALL

//================[Cooperate: 64 (OC) * 192 (GM)]=============================
//Cooperate: GM = N * IH * ((IW - ow_index) / 3 * 3)

//IWr % 3 == 0
#define winograd_f3x2_k64x192C_tex(stream, iw_index, deltaY, OH, OW, W, FH, deltaX, IH, IW, IC, OC, ph, pw, IWr) \
	winograd_f3x2_kernel_64x192C_tex<FH>\
		<<< dim3(IC>>6, ((N*IH)>>6)*(IWr/3)), dim3(16, 16), 0, stream >>>\
			(deltaY,OH,OW, W+((FH-1)*2*IC), deltaX,IH,IW, IC,OC, (FH-1-ph),(1-pw), iw_index)

//IWr % 6 == 0
#define winograd_f3x2_k64x192x6C_tex(stream, iw_index, deltaY, OH, OW, W, FH, deltaX, IH, IW, IC, OC, ph, pw, IWr) \
	winograd_f3x2_kernel_64x192x6C_tex<FH>\
		<<< dim3(IC>>6, ((N*IH)>>5)*(IWr/6)), dim3(16, 16), 0, stream >>>\
			(deltaY,OH,OW, W+((FH-1)*2*IC), deltaX,IH,IW, IC,OC, (FH-1-ph),(1-pw), iw_index)

//Split FW: IWr % 3 == 0, FW % 2 == 0
#define winograd_SFW_f3x2_k64x192C_tex(stream, iw_index, deltaY, OH, OW, W, FH, FW, deltaX, IH, IW, IC, OC, ph, pw, IWr) \
	winograd_SFW_f3x2_kernel_64x192C_tex<FW>\
		<<< dim3(IC>>6, ((N*IH)>>6)*(IWr/3)), dim3(16, 16), 0, stream >>>\
			(deltaY,OH,OW, (W+((FH-1)*FW+(FW-2))*IC),FH, deltaX,IH,IW, IC,OC, (FH-1-ph),(FW-1-pw), iw_index)

#endif


//================[Cooperate: 64(IC) * 192(GM)]===============================
//LB = 4, OC % 8 == 0, IWr % 3 == 0
#ifndef DECONV_3D_WINOGRAD_F3X2_KERNEL_64X192C_TEXTURE
#define DECONV_3D_WINOGRAD_F3X2_KERNEL_64X192C_TEXTURE

//for: Feature = (48, 48), [N, IC, OC] = [128, 128, 128]
//LB = 4: Size = 18, Time = 3.281 msec, Performace = 11781.4 GFlop/s
//for: Feature = (24, 24), [N, IC, OC] = [128, 256, 256]
//LB = 4: Size = 18, Time = 2.986 msec, Performace = 12945.3 GFlop/s
//for: Feature = (12, 12), [N, IC, OC] = [128, 512, 512]
//LB = 4: Size = 18, Time = 2.909 msec, Performace = 13288 GFlop/s

template<int FH>
__global__ void winograd_f3x2_kernel_64x192C_tex(
	...
	int iw_index)
{
	...
	const int tj0 = bj0 + Di * 3, tj4 = tj0 + 3;
	const int IWr3 = (IW - iw_index) / 3 * 3, IH_IWr3 = IH * IWr3;
	get_n_ih_iw_Temp(tj0, tn0, tih0, tiw0, IH_IWr3, IWr3); tiw0 += iw_index;//IWr % 3
	get_n_ih_iw_Temp(tj4, tn4, tih4, tiw4, IH_IWr3, IWr3); tiw4 += iw_index;
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		*(float2*)(w + 2) = *(float2*)(W + W0);//fw = 1
		*(float2*)(w    ) = *(float2*)(W + W1);//fw = 0
		...
		//Winograd transform: W(2) -> G(4); Y(4) -> D(4)
...
#pragma unroll
			...
			//load 2 group from W[OC, FH, FW, IC]
			const int W0 = (ooc*FH - fh) * 2 * IC, W1 = W0 + IC;//rotate180
			*(float2*)(w + 2) = *(float2*)(W + W0);//fw = 1
			*(float2*)(w    ) = *(float2*)(W + W1);//fw = 0
...
#pragma unroll
	...
	const int xj0 = bj0 + ((DIdx + ux) * 3), xj2 = xj0 + 12;
	get_n_ih_iw_Temp(xj0, xn0, xih0, xiw0, IH_IWr3, IWr3); xiw0 += iw_index;
	get_n_ih_iw_Temp(xj2, xn2, xih2, xiw2, IH_IWr3, IWr3); xiw2 += iw_index;
	...
	float3 x0, x1, x2, x3, x4, x5, x6, x7;
	...
	k0 = float4{ a0.x, a1.x, a2.x, a3.x }; winograd_f3x2_y_f48_64(x0, k0);
	k1 = float4{ a0.y, a1.y, a2.y, a3.y }; winograd_f3x2_y_f48_64(x1, k1);
	k2 = float4{ a0.z, a1.z, a2.z, a3.z }; winograd_f3x2_y_f48_64(x2, k2);
	k3 = float4{ a0.w, a1.w, a2.w, a3.w }; winograd_f3x2_y_f48_64(x3, k3);

	k4 = float4{ a4.x, a5.x, a6.x, a7.x }; winograd_f3x2_y_f48_64(x4, k4);
	k5 = float4{ a4.y, a5.y, a6.y, a7.y }; winograd_f3x2_y_f48_64(x5, k5);
	k6 = float4{ a4.z, a5.z, a6.z, a7.z }; winograd_f3x2_y_f48_64(x6, k6);
	k7 = float4{ a4.w, a5.w, a6.w, a7.w }; winograd_f3x2_y_f48_64(x7, k7);
	...
	*(float4*)(deltaX + X00 + IC * 2) = { x0.z, x1.z, x2.z, x3.z }; *(float4*)(deltaX + X00 + IC * 2 + 4) = { x4.z, x5.z, x6.z, x7.z };
	__syncthreads();
	...
	*(float4*)(deltaX + X20 + IC * 2) = { x0.z, x1.z, x2.z, x3.z }; *(float4*)(deltaX + X20 + IC * 2 + 4) = { x4.z, x5.z, x6.z, x7.z };
}
...
//LB = 4, OC % 8 == 0, IWr % 6 == 0
#ifndef DECONV_3D_WINOGRAD_F3X2_KERNEL_64X192X6C_TEXTURE
#define DECONV_3D_WINOGRAD_F3X2_KERNEL_64X192X6C_TEXTURE

//for: Feature = (48, 48), [N, IC, OC] = [128, 128, 128]
//LB = 4: Size = 18, Time = 3.187 msec, Performace = 12128.9 GFlop/s
//for: Feature = (24, 24), [N, IC, OC] = [128, 256, 256]
//LB = 4: Size = 18, Time = 2.916 msec, Performace = 13256.1 GFlop/s
//for: Feature = (12, 12), [N, IC, OC] = [128, 512, 512]
//LB = 4: Size = 18, Time = 2.827 msec, Performace = 13673.4 GFlop/s

template<int FH>
__global__ void winograd_f3x2_kernel_64x192x6C_tex(
	cudaTextureObject_t       deltaY, int OH, int OW,
	const float* __restrict__      W,
	      float* __restrict__ deltaX, int IH, int IW,
	int IC, int OC,
	...
	int iw_index)
{
	...
	const int bj0 = (blockIdx.y * 192);//64 * 3 = 192
	const int Dk = (tx & 7), Di = (ty << 2) + ((tx > 7) << 1);
	const int tj0 = bj0 + Di * 3;
	const int IWr3 = IW - iw_index, IH_IWr3 = IH * IWr3;
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		*(float2*)(w    ) = *(float2*)(W + W1);//fw = 0
		...
		tY2 = IF_int(ly2, tY2, -1); tY3 = IF_int(ly3, tY3, -1);
		tY4 = IF_int(ly4, tY4, -1); tY5 = IF_int(ly5, tY5, -1);
		tY6 = IF_int(ly6, tY6, -1);
		y[0] = tex1Dfetch<float>(deltaY, tY0); y[1] = tex1Dfetch<float>(deltaY, tY1);
		y[2] = tex1Dfetch<float>(deltaY, tY2); y[3] = tex1Dfetch<float>(deltaY, tY3);
		y[4] = tex1Dfetch<float>(deltaY, tY4); y[5] = tex1Dfetch<float>(deltaY, tY5);
		y[6] = tex1Dfetch<float>(deltaY, tY6);
		...
		float d0[4]{ y[0] - y[2], y[1] + y[2], y[2] - y[1], y[1] - y[3] };
		float d1[4]{ y[3] - y[5], y[4] + y[5], y[5] - y[4], y[4] - y[6] };
...
//Split SFW: LB = 4, OC % 8 == 0, IWr % 3 == 0, FW % 2 == 0
//LB = 4, OC % 8 == 0, IWr % 3 == 0
#ifndef DECONV_3D_WINOGRAD_SFW_F3X2_KERNEL_64X192C_TEXTURE
#define DECONV_3D_WINOGRAD_SFW_F3X2_KERNEL_64X192C_TEXTURE

//for: [FH, FW] = 4, Feature = (24, 24), [N, IC, OC] = [64, 256, 256],
//LB = 4: Size = 72, Time = 11.003 msec, Performace = 14052.4 GFlop/s

template<int FW>
__global__ void winograd_SFW_f3x2_kernel_64x192C_tex(
	cudaTextureObject_t       deltaY, int OH, int OW,
	...
	int iw_index)
{
	...
	//======[compute area1: local]======================================================
	float w[4], y[8]; const int FHW = FH * FW;
	for (int fhw = 0; fhw < FHW; fhw += 2) {
		const int fh = fhw / FW, fw = fhw - fh * FW;

		//load 2 group from W[OC, FH, FW, IC]
		const int W0 = (-fh * FW - fw)*IC, W1 = W0 + IC;//rotate180
		*(float2*)(w + 2) = *(float2*)(W + W0);//fw = 1
		*(float2*)(w    ) = *(float2*)(W + W1);//fw = 0
...
#pragma unroll
			...
			const int W0 = ((ooc*FH - fh)*FW - fw)*IC, W1 = W0 + IC;//rotate180
			*(float2*)(w + 2) = *(float2*)(W + W0);//fw = 1
			*(float2*)(w    ) = *(float2*)(W + W1);//fw = 0
