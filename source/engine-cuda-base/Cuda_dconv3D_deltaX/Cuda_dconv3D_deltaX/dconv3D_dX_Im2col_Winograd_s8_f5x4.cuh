...
#ifndef DECONV_3D_DX_WINOGRAD_S8_F5X4_H
#define DECONV_3D_DX_WINOGRAD_S8_F5X4_H

//(1) sh = sw = 1
//(2) FW = 4
//(3) IW % 5: group = 5 elements
#ifndef DECONV_3D_DX_WINOGRAD_F5X4_CALL
#define DECONV_3D_DX_WINOGRAD_F5X4_CALL

//<1> (opw <= 2) -> (FW - 1 - pw <= 2) -> (pw >= 1)
//<2> (IW - OW - opw + FW - 1 <= 3) -> (IW - OW + pw <= 3) -> (OW - IW - pw + 3 >= 0)

//================[Standard: 64(IC) * 160(GM)]===============================
//IW % 5 == 0
#define winograd_f5x4_k64x160_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f5x4_kernel_64x160_tex<FH>\
		<<< dim3(IC>>6, ((N*IH)>>5)*(IW/5)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*4*IC), deltaX,IH,IW, IC,OC, (FH-1-ph),(3-pw))

//IW % 5 == 0, pw >= 1
#define winograd_f5x4_k64x160_p1_tex(stream, deltaY, OH, OW, W, FH, deltaX, IH, IW, N, IC, OC, ph, pw) \
	winograd_f5x4_kernel_64x160_p1_tex<FH>\
		<<< dim3(IC>>6, ((N*IH)>>5)*(IW/5)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, W+((FH-1)*4*IC), deltaX,IH,IW, IC,OC, (FH-1-ph),(3-pw))

//================[Cooperate: 64(IC) * 160(GM)]==============================
#define winograd_SFW_f5x4_k64x160C_tex(stream, iw_index, deltaY, OH, OW, W, FH, FW, deltaX, IH, IW, N, IC, OC, ph, pw, IWr) \
	winograd_SFW_f5x4_kernel_64x160C_tex<FW>\
		<<< dim3(IC>>6, ((N*IH)>>5)*(IWr/5)), dim3(16, 16), 0, stream>>>\
			(deltaY,OH,OW, (W+((FH-1)*FW+(FW-4))*IC),FH, deltaX,IH,IW, IC,OC, (FH-1-ph),(FW-1-pw), iw_index)
...
//================[Standard: 64(IC) * 160(GM)]===============================
//LB = 4, OC % 8 == 0, IW % 5 == 0
#ifndef DECONV_3D_DX_WINOGRAD_F5X4_KERNEL_64X160_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F5X4_KERNEL_64X160_TEXTURE

//for: Feature = (80, 80), [N, IC, OC] = [128, 64, 64]
//LB = 4: Size = 50, Time = 5.607 msec, Performace = 19150 GFlop/s
//for: Feature = (40, 40), [N, IC, OC] = [128, 128, 128]
//LB = 4: Size = 50, Time = 5.314 msec, Performace = 20205.9 GFlop/s
//for: Feature = (20, 20), [N, IC, OC] = [128, 256, 256]
//LB = 4: Size = 50, Time = 5.133 msec, Performace = 20918.4 GFlop/s

template<int FH>
__global__ void winograd_f5x4_kernel_64x160_tex(
	...
	int oph, int opw)//sh = sw = 1
{
	...
	W += (Gk * (FH << 2) * IC) + tic0;//W[Gk, 0, 0, tic0]

	//prepare for deltaY[N, OH, OW, OC]
	const int bj0 = (blockIdx.y * 160);//32 * 5 = 160
	const int Yk = (tx & 7), Yi = (ty << 1) + (tx > 7);//[8, 32]
	const int tj0 = bj0  + Yi * 5;
	const int IW5 = IW / 5 * 5, IH_IW5 = IH * IW5;
	get_n_ih_iw_Temp(tj0, tn0, tih0, tiw0, IH_IW5, IW5);
	...
	const int xj0 = bj0 + (DIdx + (ux >> 1)) * 5, xj1 = xj0 + 20;
	get_n_ih_iw_Temp(xj0, xn0, xih0, xiw0, IH_IW5, IW5);
	get_n_ih_iw_Temp(xj1, xn1, xih1, xiw1, IH_IW5, IW5);
	const int X00 = ((xn0*IH + xih0)*IW + xiw0)*IC + xic0;//X00 = xj0 * IC + xic0;
	const int X10 = ((xn1*IH + xih1)*IW + xiw1)*IC + xic0;//X10 = X00 + IC * 20;

	//======[compute area1: local]======================================================
	const int Di = (Yi + (Yk >> 2 << 4)) & 31;//avoid bank conflict (1 / 2)
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		const int W0 = -(fh << 2) * IC;//rotate180
		const int W1 = W0 + IC, W2 = W0 + (IC << 1), W3 = W0 + IC * 3;
		float2 w3 = *(float2*)(W + W0);
		float2 w2 = *(float2*)(W + W1);
		float2 w1 = *(float2*)(W + W2);
		float2 w0 = *(float2*)(W + W3);

		//Winograd transform: W(4) -> G(8); Y(8) -> D(8)
...
#pragma unroll
			...
			//load 2 group from W[OC, FH, FW, IC]
			const int W0 = ((ooc*FH - fh) << 2) * IC;//rotate180
			const int W1 = W0 + IC, W2 = W0 + (IC << 1), W3 = W0 + IC * 3;
			float2 w3 = *(float2*)(W + W0);
			float2 w2 = *(float2*)(W + W1);
			float2 w1 = *(float2*)(W + W2);
...
#pragma unroll
	...
	float2 a[16]; float x0[5], x1[5], x2[5], x3[5];
	__syncthreads();
	...
	*(float4*)(&Xs[ux][uy][6]) = { v6.x, v6.y, v7.x, v7.y };
	...
	winograd_f5x4_Y_vec(x0, a, x);//{ic0 / ic4} * { j0 - j3 }
	winograd_f5x4_Y_vec(x1, a, y);//{ic1 / ic5} * { j0 - j3 }
	__syncthreads();
	...
	*(float4*)(&Xs[ux][uy][6]) = { v6.z, v6.w, v7.z, v7.w };
	...
	a[4] = Xs[4][uy][ux]; a[5] = Xs[5][uy][ux];
	a[6] = Xs[6][uy][ux]; a[7] = Xs[7][uy][ux];
	winograd_f5x4_Y_vec(x2, a, x);//{ic2 / ic6} * { j0 - j3 }
	winograd_f5x4_Y_vec(x3, a, y);//{ic3 / ic7} * { j0 - j3 }
	...
	*(float4*)(&Xs[ux][uy][6]) = { v14.x, v14.y, v15.x, v15.y };
	...
	a[4] = Xs[4][uy][ux]; a[5] = Xs[5][uy][ux];
	a[6] = Xs[6][uy][ux]; a[7] = Xs[7][uy][ux];
	winograd_f5x4_Y_vec(x0, a, x);//{ic0 / ic4} * { j4 - j7 }
	winograd_f5x4_Y_vec(x1, a, y);//{ic1 / ic5} * { j4 - j7 }
	__syncthreads();
	...
	*(float4*)(&Xs[ux][uy][6]) = { v14.z, v14.w, v15.z, v15.w };
	...
	a[4] = Xs[4][uy][ux]; a[5] = Xs[5][uy][ux];
	a[6] = Xs[6][uy][ux]; a[7] = Xs[7][uy][ux];
	winograd_f5x4_Y_vec(x2, a, x);//{ic2 / ic6} * { j4 - j7 }
	winograd_f5x4_Y_vec(x3, a, y);//{ic3 / ic7} * { j4 - j7 }
...
//LB = 4, OC % 8 == 0, IW % 5 == 0, pw >= 1
#ifndef DECONV_3D_DX_WINOGRAD_F5X4_KERNEL_64X160_P1_TEXTURE
#define DECONV_3D_DX_WINOGRAD_F5X4_KERNEL_64X160_P1_TEXTURE
...
template<int FH>
__global__ void winograd_f5x4_kernel_64x160_p1_tex(
	cudaTextureObject_t       deltaY, int OH, int OW, 
	const float* __restrict__      W,
	      float* __restrict__ deltaX, int IH, int IW, 
	int IC, int OC,
	int oph, int opw)//sh = sw = 1
{
	...
	//======[compute area1: local]======================================================
	const int Di = (Yi + (Yk >> 1 << 3)) & 31;//avoid bank conflict (1 / 2)
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		bool ly5 = lh0 && (tow0 >= -5) && (tow0 + 5 < OW);//opw = +1
		bool ly6 = lh0 && (tow0 >= -6) && (tow0 + 6 < OW);//opw = +2
		bool ly7 = lh0 && (tow0 >= -7) && (tow0 + 7 < OW);//opw = +3
		int tY0 = Y0 + fh * OW * OC;
		int tY1 = tY0 + OC, tY2 = tY0 + (OC << 1);
		int tY3 = tY0 + OC * 3, tY4 = tY0 + (OC << 2);
		int tY5 = tY0 + OC * 5, tY6 = tY0 + OC * 6, tY7 = tY0 + OC * 7;
		...
		tY4 = IF_int(lh0, tY4, -1);
		tY5 = IF_int(ly5, tY5, -1); tY6 = IF_int(ly6, tY6, -1); tY7 = IF_int(ly7, tY7, -1);
		y[0] = tex1Dfetch<float>(deltaY, tY0); y[1] = tex1Dfetch<float>(deltaY, tY1);
		y[2] = tex1Dfetch<float>(deltaY, tY2); y[3] = tex1Dfetch<float>(deltaY, tY3);
		y[4] = tex1Dfetch<float>(deltaY, tY4); y[5] = tex1Dfetch<float>(deltaY, tY5);
		y[6] = tex1Dfetch<float>(deltaY, tY6); y[7] = tex1Dfetch<float>(deltaY, tY7);
...
//================[Cooperate: 64(IC) * 160(GM)]==============================
//LB = 4, OC % 8 == 0, IW % 5 == 0, FW % 4 == 0
#ifndef DECONV_3D_DX_WINOGRAD_SFW_F5X4_KERNEL_64X160C_TEXTURE
#define DECONV_3D_DX_WINOGRAD_SFW_F5X4_KERNEL_64X160C_TEXTURE

//FH = FW = 8
//for: Feature = (40, 40), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 100, Time = 10.652 msec, Performace = 20160.4 GFlop/s
//for: Feature = (20, 20), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 100, Time = 10.848 msec, Performace = 19796.1 GFlop/s

template<int FW>
__global__ void winograd_SFW_f5x4_kernel_64x160C_tex(
	cudaTextureObject_t       deltaY, int OH, int OW, 
	...
	int iw_index)
{
	...
	const int Yk = (tx & 7), Yi = (ty << 1) + (tx > 7);//[8, 32]
	const int tj0 = bj0  + Yi * 5;
	const int IWr5 = (IW - iw_index) / 5 * 5, IH_IWr5 = IH * IWr5;
	get_n_ih_iw_Temp(tj0, tn0, tih0, tiw0, IH_IWr5, IWr5); tiw0 += iw_index;
	const int toh0 = tih0 - oph, tow0 = tiw0 - opw;
	const int Y0 = ((tn0*OH + toh0)*OW + tow0)*OC + Yk;//deltaY[tn0, toh0, tow0, Yk]
	...
	const int xic0 = bic0 + GIdx + ((ux & 1) << 2);
	const int xj0 = bj0 + (DIdx + (ux >> 1)) * 5, xj1 = xj0 + 20;
	get_n_ih_iw_Temp(xj0, xn0, xih0, xiw0, IH_IWr5, IWr5); xiw0 += iw_index;
	get_n_ih_iw_Temp(xj1, xn1, xih1, xiw1, IH_IWr5, IWr5); xiw1 += iw_index;
	const int X00 = ((xn0*IH + xih0)*IW + xiw0)*IC + xic0;//X00 = xj0 * IC + xic0;
	const int X10 = ((xn1*IH + xih1)*IW + xiw1)*IC + xic0;//X10 = X00 + IC * 20;

	//======[compute area1: local]======================================================
	const int Di = (Yi + (Yk >> 2 << 4)) & 31;//avoid bank conflict (1 / 2)
	float g0[8], g1[8], y[8], d[8]; char ly[8]; const int FHW = FH * FW;
	for (int fhw = 0; fhw < FHW; fhw += 4) {
		const int fh = fhw / FW, fw = fhw - fh * FW;

		//load 1 group from deltaY[N, OH, OW, OC]
		bool lh0 = (toh0 >= -fh) && (toh0 + fh < OH);
		ly[0] = lh0 && (tow0 + fw >=  0) && (tow0 + fw     < OW);
		ly[1] = lh0 && (tow0 + fw >= -1) && (tow0 + fw + 1 < OW);
		ly[2] = lh0 && (tow0 + fw >= -2) && (tow0 + fw + 2 < OW);
		ly[3] = lh0 && (tow0 + fw >= -3) && (tow0 + fw + 3 < OW);
		ly[4] = lh0 && (tow0 + fw >= -4) && (tow0 + fw + 4 < OW);
		ly[5] = lh0 && (tow0 + fw >= -5) && (tow0 + fw + 5 < OW);
		ly[6] = lh0 && (tow0 + fw >= -6) && (tow0 + fw + 6 < OW);
		ly[7] = lh0 && (tow0 + fw >= -7) && (tow0 + fw + 7 < OW);
