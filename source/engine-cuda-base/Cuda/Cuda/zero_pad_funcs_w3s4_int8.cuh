...
#ifndef ZERO_PAD_FUNCS_W3S4_int8_H
#define ZERO_PAD_FUNCS_W3S4_int8_H

//[1] W3: width  = 3
//[2] S4: stride = 4
//designed for pictures(JPG) with 3 channels but padding to 4
//height % 4 == 0, width = 3, stride = 4
//A[height, 3] -> B[height, 4]
...
//Size = 3.5, Time = 0.3 mesc, Speed = 11.3932 GB/s
...
//Size = 3.5, Time = 0.35 mesc, Speed = 9.76562 GB/s
...
//height % 16 == 0
#ifndef ZERO_PAD_W3S4_HX16
#define ZERO_PAD_W3S4_HX16

//Size = 3.5, Time = 0.46 mesc, Speed = 7.43037 GB/s
void __zeroPad_W3S4_Hx16(const char *A, char *B, int height)
{
	int_4* A_128bit = (int_4*)A;//128 bits
	int_4* B_128bit = (int_4*)B;
	
	char   rbuf[48];//3*16 = 48 elems
	char_4 wbuf[16];//4*16 = 64 elems
	memset(wbuf, 0, sizeof(char_4) * 16);

	for (int i = 0; i < height; i += 16)//3*32 = 96 element -> 4*32 = 128 element
	{
		...
		*(char_3*)(wbuf + 0) = *(char_3*)(rbuf    );//0 -> 11
		*(char_3*)(wbuf + 1) = *(char_3*)(rbuf + 3);
		*(char_3*)(wbuf + 2) = *(char_3*)(rbuf + 6);
		*(char_3*)(wbuf + 3) = *(char_3*)(rbuf + 9);

		*(char_3*)(wbuf + 4) = *(char_3*)(rbuf + 12);//12 -> 23
		*(char_3*)(wbuf + 5) = *(char_3*)(rbuf + 15);
		*(char_3*)(wbuf + 6) = *(char_3*)(rbuf + 18);
		*(char_3*)(wbuf + 7) = *(char_3*)(rbuf + 21);

		*(char_3*)(wbuf +  8) = *(char_3*)(rbuf + 24);//24 -> 35
		*(char_3*)(wbuf +  9) = *(char_3*)(rbuf + 27);
		*(char_3*)(wbuf + 10) = *(char_3*)(rbuf + 30);
		*(char_3*)(wbuf + 11) = *(char_3*)(rbuf + 33);

		*(char_3*)(wbuf + 12) = *(char_3*)(rbuf + 36);//36 -> 47
		*(char_3*)(wbuf + 13) = *(char_3*)(rbuf + 39);
		*(char_3*)(wbuf + 14) = *(char_3*)(rbuf + 42);
		*(char_3*)(wbuf + 15) = *(char_3*)(rbuf + 45);
...
//Size = 3.5, Time = 0.72 mesc, Speed = 4.74718 GB/s
...
//Size = 3.5, Time = 1.27 mesc, Speed = 2.69131 GB/s
...
void __zeroPad_W3S4(const char *A, char *B, int height)
{
	...
	if (height64 > 0) {//unit: = 64
		__zeroPad_W3S4_Hx64(A, B, height64);
		height &= 63; if (height == 0) return;//height %= 64
	...
	if (height32 > 0) {
		...
		height &= 31; if (height == 0) return;//height %= 32
		A += (height32 * 3); B += (height32 << 2);//[3, 4]
	}

	int height16 = (height >> 4) << 4;
	if (height16 > 0) {
		__zeroPad_W3S4_Hx16(A, B, height16);
		height &= 15; if (height == 0) return;//height %= 16
		A += (height16 * 3); B += (height16 << 2);//[3, 4]
	}

	int height8 = (height >> 3) << 3;
	if (height8 > 0) {
		__zeroPad_W3S4_Hx8(A, B, height);
		height &= 7; if (height == 0) return;//height %= 8
	...
	if (height4 > 0) {
		...
		height &= 3; if (height == 0) return;//height %= 4
