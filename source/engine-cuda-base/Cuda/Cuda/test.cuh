...
void test_v3(int nIter, int height, int width)
{
	...
	//__zeroPad_W3S4_Hx16(A, B2, height);
	//__zeroPad_W3S4_Hx8(A, B2, height);
	__zeroPad_W3S4_Hx4(A, B2, height);
	...
	{
		//__zeroPad_W3S4(A, B2, height);
		//__zeroPad_W3S4_Hx64(A, B2, height);
		//__zeroPad_W3S4_Hx32(A, B2, height);
		//__zeroPad_W3S4_Hx16(A, B2, height);
		//__zeroPad_W3S4_Hx8(A, B2, height);
		__zeroPad_W3S4_Hx4(A, B2, height);
	}
	...
	int div = (end - start);
	float time = 1.0f * div / nIter;
	int data_size = (length + lengthv) * sizeof(char);
	float speed = (1.0f *(data_size) / (1 << 30)) / (time*1e-3);
	cout << "Size = " << data_size / 1024.0f  / 1024 << ", "
		 << "Time = " << time << " mesc, "
		 << "Speed = " << speed << " GB/s" << endl;
}
...
void test()
{
	...
	int C = 3;
	test_v3(100, 512 * 32 * 32, C);

	//for (int h = 1; h < 1024; h++) test_v3(-1, h, 3);
}
