...
#ifndef FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2_H
#define FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2_H
...
//[16, 4]
#define field_affine_with_leakyRelu_deltaAB_v2_k4_small(stream, deltaY, k, X, A, B, N, M, deltaA, deltaB) \
	field_affine_with_leakyRelu_deltaAB_v2_kernel_4<3, 3>\
		<<< dim3(MIN_int32((M + 31)>>5, GRID_MAX),\
                 MIN_int32((N + 63)>>6, GRID_MAX)),\
            dim3(8, 8), 0, stream>>>\
			(deltaY, k, X, A, B, N, M, deltaA, deltaB, width, stride)

//LTX >=2
#define field_affine_with_leakyRelu_deltaAB_v2_k4(stream, LBY, LBX, LTY, LTX, deltaY, k, X, A, B, N, M, deltaA, deltaB) \
	field_affine_with_leakyRelu_deltaAB_v2_kernel_4<LBY, LBX>\
...
#ifndef FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2_KERNEL_4
#define FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2_KERNEL_4
...
__global__ void field_affine_with_leakyRelu_deltaAB_v2_kernel_4(
...
//======[integration]========================================
#ifndef FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2_STAGE
#define FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2_STAGE
...
{
	if (M > 15) {
		if (N > 63) { field_affine_with_leakyRelu_deltaAB_v2_k4(stream, 3, 2, 3, 2, deltaY, k, X, A, B, N, M, deltaA, deltaB); return; }//[64, 16]
		if (N > 31) { field_affine_with_leakyRelu_deltaAB_v2_k4(stream, 3, 2, 2, 2, deltaY, k, X, A, B, N, M, deltaA, deltaB); return; }//[32, 16]
		if (N > 15) { field_affine_with_leakyRelu_deltaAB_v2_k4(stream, 3, 2, 1, 2, deltaY, k, X, A, B, N, M, deltaA, deltaB); return; }//[16, 16]
	}
	if (M > 7) {
		if (N > 127) { field_affine_with_leakyRelu_deltaAB_v2_k4(stream, 4, 1, 3, 2, deltaY, k, X, A, B, N, M, deltaA, deltaB); return; }//[ 64, 8]
		if (N >  63) { field_affine_with_leakyRelu_deltaAB_v2_k4(stream, 4, 1, 2, 2, deltaY, k, X, A, B, N, M, deltaA, deltaB); return; }//[ 32, 8]
		if (N >  31) { field_affine_with_leakyRelu_deltaAB_v2_k4(stream, 4, 1, 1, 2, deltaY, k, X, A, B, N, M, deltaA, deltaB); return; }//[ 16, 8]
	}
	field_affine_with_leakyRelu_deltaAB_v2_k4_small(stream, deltaY, k, X, A, B, N, M, deltaA, deltaB);
}

#endif 


#ifndef FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2
#define FIELD_AFFINE_WITH_LEAKY_RELU_DELTA_AB_V2

int __field_affine_with_leakyRelu_deltaAB_v2(JNIEnv *env,
	...
	int partNum)
{
	...
	if (nextN <= partNum) {//V.address = NULL, only 1 stage, write result to Y
		__field_affine_with_leakyRelu_deltaAB_v2_stage(stream1,
			deltaY, k, X, A, B, N, M,
			deltaA,
			deltaB,
			width, stride);
