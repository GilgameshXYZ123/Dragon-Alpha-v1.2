...
#ifndef FIELD_QUADRATIC_DUAL_KERNEL_4_SHFL
#define FIELD_QUADRATIC_DUAL_KERNEL_4_SHFL

//warp_shfl: LTX >= 2, LBX >= 3
#define field_quadratic_dual4_shfl(stream, LBX, LBY, LTX, LTY, X1, X2, k11, k12, k22, k1, k2, C, N, M, V, width, stride) \
	field_quadratic_dual_kernel_4_shfl<LBX, LBY>\
		<<< dim3(N>>LBX>>LTX, M>>LBY>>LTY), dim3(1<<LBX, 1<<LBY), 0, stream >>>\
			(X1, X2, k11, k12, k22, k1, k2, C, N, M, V, width, stride)

template<int LBX, int LBY>
__global__ void field_quadratic_dual_kernel_4_shfl(
	...
	int width, int stride)
{
	...
	for (int x4 = offsetX << 2; x4 < M4; x4 += stepX4)
	{
		...
		{
			const int xoffset = y * M + x4;
			float4 x1 = *(float4*)(X1 + xoffset);//X1[y, x4]
			float4 x2 = *(float4*)(X2 + xoffset);//X2[y, x4]

			float4 a;
			a.x = k11 * (x1.x * x1.x) + k12 * (x1.x * x2.x) + k22 * (x2.x * x2.x) + k1 * x1.x + k2 * x2.x + C - c.x;
			a.y = k11 * (x1.y * x1.y) + k12 * (x1.y * x2.y) + k22 * (x2.y * x2.y) + k1 * x1.y + k2 * x2.y + C - c.y;
			a.z = k11 * (x1.z * x1.z) + k12 * (x1.z * x2.z) + k22 * (x2.z * x2.z) + k1 * x1.z + k2 * x2.z + C - c.z;
			a.w = k11 * (x1.w * x1.w) + k12 * (x1.w * x2.w) + k22 * (x2.w * x2.w) + k1 * x1.w + k2 * x2.w + C - c.w;
