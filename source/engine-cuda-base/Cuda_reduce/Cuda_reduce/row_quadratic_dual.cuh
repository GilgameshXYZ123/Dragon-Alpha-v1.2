	...
	int width, int stride)
{
	...
	const int Yoffset = y * M, stepAB = stepY * M;
	for (A += Yoffset, B += Yoffset; y < N; y += stepY)
	{
		...
		for (int x = Xoffset; x < M; x += stepX4)
		{
			...
			float4 yv;
			yv.x = (k11 * a.x*a.x) + (k12 * a.x*b.x) + (k22 * b.x*b.x) + (k1 * a.x) + (k2 * b.x) + C;
			yv.y = (k11 * a.y*a.y) + (k12 * a.y*b.y) + (k22 * b.y*b.y) + (k1 * a.y) + (k2 * b.y) + C;
			yv.z = (k11 * a.z*a.z) + (k12 * a.z*b.z) + (k22 * b.z*b.z) + (k1 * a.z) + (k2 * b.z) + C;
			yv.w = (k11 * a.w*a.w) + (k12 * a.w*b.w) + (k22 * b.w*b.w) + (k1 * a.w) + (k2 * b.w) + C;
			within_width4(yv, x, stride, width);
			Kahan_sum4(v, yv, c);//v += a.x + a.y + a.z + a.w;
		}
		As[ty][tx] = v;
		A += stepAB;//A[Y + stepY][Xoffset]
		B += stepAB;//B[Y + stepY][Xoffset]
		__syncthreads();
