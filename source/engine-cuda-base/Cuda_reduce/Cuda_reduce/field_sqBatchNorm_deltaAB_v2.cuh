	...
	int width, int stride) 
{
	...
	for (int x4 = offsetX << 2; x4 < M4; x4 += stepX4)
	{
		...
		{
			const int src_offset = y * M + x4;//[y, x4]
		...
		int yIdx;
		float4 sc[2]{//with same tx: overlap the error in same field
		...
		if (LBY >= 6) {
			yIdx = ((ty & 31) << 1) + (ty >> 5);
			if (ty < 64) {//128 -> 64
				float4 v1 = As[tx][yIdx], v2 = As[tx][yIdx + 64], c = sc[(yIdx & 1)];
				Kahan_simdAdd4(v1, v2, c); As[tx][yIdx] = v1;
			}
			__syncthreads();
		}
		if (LBY >= 5) {
			yIdx = ((ty & 15) << 1) + (ty >> 4);
			if (ty < 32) {//64 -> 32
				float4 v1 = As[tx][yIdx], v2 = As[tx][yIdx + 32], c = sc[(yIdx & 1)];
				Kahan_simdAdd4(v1, v2, c); As[tx][yIdx] = v1;
			}
			__syncthreads();
		}
		if (LBY >= 4) {//32 -> 16
			yIdx = ((ty & 7) << 1) + (ty >> 3);
			if (ty < 16) {
				float4 v1 = As[tx][yIdx], v2 = As[tx][yIdx + 16], c = sc[(yIdx & 1)];
				Kahan_simdAdd4(v1, v2, c); As[tx][yIdx] = v1;
			}
			__syncthreads();
		}
		...
		if (ty < 8) {//16 -> 8
			float4 v1 = As[tx][yIdx], v2 = As[tx][yIdx + 8], c = sc[(yIdx & 1)];
			Kahan_simdAdd4(v1, v2, c); As[tx][yIdx] = v1;
		}
		__syncthreads();
		...
		yIdx = ((ty & 1) << 1) + (ty >> 1);
		if (ty < 4) {//8 -> 4
			float4 v1 = As[tx][yIdx], v2 = As[tx][yIdx + 4], c = sc[(yIdx & 1)];
			Kahan_simdAdd4(v1, v2, c); As[tx][yIdx] = v1;
		}
		__syncthreads();  
		...
		yIdx = ((ty & 0) << 1) + (ty >> 0);
		if (ty < 2) {
			float4 v1 = As[tx][yIdx], v2 = As[tx][yIdx + 2], c = sc[(yIdx & 1)];
			Kahan_simdAdd4(v1, v2, c);//v1 += v2

			float2 dB = float2{ v1.x, v1.y };//deltaXp1 = deltaB
			float2 dA = float2{ v1.z, v1.w };//deltaXp2 = deltaA
