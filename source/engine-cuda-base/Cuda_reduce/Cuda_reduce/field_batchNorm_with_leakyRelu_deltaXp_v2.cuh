...
#ifndef FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_H
#define FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_H
...
#ifndef FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_CALL
#define FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_CALL

//[16, 4]
#define field_batchNorm_with_leakyRelu_deltaXp_v2_k4_small(stream, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2) \
	field_batchNorm_with_leakyRelu_deltaXp_v2_kernel_4<3, 3>\
		<<< dim3(MIN_int32((M + 31)>>5, GRID_MAX),\
                 MIN_int32((N + 63)>>6, GRID_MAX)),\
            dim3(8, 8), 0, stream>>>\
			(deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2, width, stride)

//LTX >=2
#define field_batchNorm_with_leakyRelu_deltaXp_v2_k4(stream, LBY, LBX, LTY, LTX, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2) \
	field_batchNorm_with_leakyRelu_deltaXp_v2_kernel_4<LBY, LBX>\
...
#ifndef FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_KERNEL_4
#define FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_KERNEL_4
...
__global__ void field_batchNorm_with_leakyRelu_deltaXp_v2_kernel_4(
	...
	int width, int stride)
{
	...
	for (int x4 = offsetX << 2; x4 < M4; x4 += stepX4)
	{
		...
		for (int y = offsetY; y < N; y += stepY)
		{
			...
			int4 flag;//flag =  X > X_mean
			flag.x = (xv.x > x_mean.x);
			flag.y = (xv.y > x_mean.y);
			flag.z = (xv.z > x_mean.z);
			flag.w = (xv.w > x_mean.w);
...
//======[integration]======================================================
#ifndef FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_STAGE
#define FIELD_BATCH_NORM_WITH_LEAKY_RELU_DELTA_XP_V2_STAGE

void __field_batchNorm_with_leakyRelu_deltaXp_v2_stage(cudaStream_t stream,
...
{
	if (M > 15) {
		if (N > 63) { field_batchNorm_with_leakyRelu_deltaXp_v2_k4(stream, 3, 2, 3, 2, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2); return; }//[64, 16]
		if (N > 31) { field_batchNorm_with_leakyRelu_deltaXp_v2_k4(stream, 3, 2, 2, 2, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2); return; }//[32, 16]
		if (N > 15) { field_batchNorm_with_leakyRelu_deltaXp_v2_k4(stream, 3, 2, 1, 2, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2); return; }//[16, 16]
	}
	if (M > 7) {
		if (N > 127) { field_batchNorm_with_leakyRelu_deltaXp_v2_k4(stream, 4, 1, 3, 2, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2); return; }//[ 64, 8]
		if (N >  63) { field_batchNorm_with_leakyRelu_deltaXp_v2_k4(stream, 4, 1, 2, 2, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2); return; }//[ 32, 8]
		if (N >  31) { field_batchNorm_with_leakyRelu_deltaXp_v2_k4(stream, 4, 1, 1, 2, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2); return; }//[ 16, 8]
	}
	field_batchNorm_with_leakyRelu_deltaXp_v2_k4_small(stream, deltaY, k, X, X_mean, X_var, eps, N, M, deltaXp1, deltaXp2);
}
	...
	int partNum)
{
	...
	if (nextN <= partNum) {//V.address = NULL, only 1 stage, write result to Y
		__field_batchNorm_with_leakyRelu_deltaXp_v2_stage(stream1,
			deltaY, k, X, X_mean, X_var, eps, N, M,
			deltaXp1,
			deltaXp2,
			width, stride);
		return nextN;
