...
#ifndef FIELD_LINEAR_KERNEL_4_SHFL
#define FIELD_LINEAR_KERNEL_4_SHFL

//warp_shfl: LTX >= 2, LBX >= 3
#define field_linear4_shfl(stream, LBX, LBY, LTX, LTY, A, alpha, beta, N, M, V, width, stride) \
	field_linear_kernel_4_shfl<LBX, LBY>\
		<<< dim3(N>>LBX>>LTX, M>>LBY>>LTY), dim3(1<<LBX, 1<<LBY), 0, stream >>>\
			(A, alpha, beta, N, M, V, width, stride)

template<int LBX, int LBY>
__global__ void field_linear_kernel_4_shfl(
...
{
	if (M > 15) {
		if (N >  63) { field_linear4(stream, 3, 2, 3, 2, A, alpha, beta, N, M, V, width, stride); return; }//[6, 4: 64, 16]
		if (N >  31) { field_linear4(stream, 3, 2, 2, 2, A, alpha, beta, N, M, V, width, stride); return; }//[5, 4: 32, 16]
		if (N >  15) { field_linear4(stream, 3, 2, 1, 2, A, alpha, beta, N, M, V, width, stride); return; }//[4, 4: 16, 16]
	}
	if (M > 7) {
