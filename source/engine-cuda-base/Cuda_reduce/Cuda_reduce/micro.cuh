...
//int straight_nextLengthV(int lengthv) { return (lengthv > 8192 ? (lengthv >> 13) : 1); }
inline int straight_nextLengthV(int lengthv) {
	return (lengthv > 8192 ? MIN_int32((lengthv >> 13), GRID_MAX) : 1);
}


//int row_nextM(int M) { return (M > 255 ? (M >> 8) : 1); }
inline int row_nextM(int M) {
	return (M > 255 ? MIN_int32((M >> 8), GRID_MAX) : 1);
}


inline int center_nextN(int N, int M) {
	if (M > 15) {
...
inline int field_nextN(int N, int M) {
	if (M > 15) {
		if (N >  63) return MIN_int32(N >> 6, GRID_MAX);
		if (N >  15) return 1;
	}
...
#define __SIMD2_shfl_down_sync(a, mask, b, delta) {\
	a.x = __shfl_down_sync(mask, b.x, delta);\
	a.y = __shfl_down_sync(mask, b.y, delta);}

#define __SIMD4_shfl_down_sync(a, mask, b, delta) {\
	a.x = __shfl_down_sync(mask, b.x, delta);\
	a.y = __shfl_down_sync(mask, b.y, delta);\
	a.z = __shfl_down_sync(mask, b.z, delta);\
	a.w = __shfl_down_sync(mask, b.w, delta);}


#ifndef KUHAN_MICRO
#define KUHAN_MICRO

#define Kahan_simdAdd2(v, a, c)	{\
	float2 dv;\
	dv.x = a.x - c.x;\
	dv.y = a.y - c.y;\
	float2 t;\
...
#ifndef WARP_SIMD_SUM
#define WARP_SIMD_SUM
...
#ifndef WARP_SIMD_SUM_WITH_INDEX
#define WARP_SIMD_SUM_WITH_INDEX
