...
#ifndef CENTER_QUADRATIC_H
#define CENTER_QUADRATIC_H
...
#ifndef CENTER_QUADRATIC_CALL
#define CENTER_QUADRATIC_CALL

//[16, 4]
#define center_quadratic4_small(stream, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride) \
	center_quadratic_kernel_4<3, 3>\
		<<< dim3(MIN_int32((M + 31)>>5, GRID_MAX),\
                 MIN_int32((N + 63)>>6, GRID_MAX)),\
            dim3(8, 8), 0, stream>>>\
			(X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride)

//LTX >=2
#define center_quadratic4(stream, LBY, LBX, LTY, LTX, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride) \
	center_quadratic_kernel_4<LBY, LBX>\
...
#ifndef CENTER_QUADRATIC_KERNEL_4
#define CENTER_QUADRATIC_KERNEL_4

template<int LBY, int LBX>
__global__ void center_quadratic_kernel_4(
	...
	int width, int stride)
{
	...
	for (int x4 = X0 << 2; x4 < M4; x4 += stepX4)//x4 -> M[d0, d2]
	{
		...
		for (int d1 = Y0; d1 < N; d1 += stepY)//y -> N[d1]
		{
			...
			float4 a;//<1> a = alpha*X^2 + beta*X + gamma
			a.x = alpha * x.x * x.x + beta * x.x + gamma;
			a.y = alpha * x.y * x.y + beta * x.y + gamma;
			a.z = alpha * x.z * x.z + beta * x.z + gamma;
			a.w = alpha * x.w * x.w + beta * x.w + gamma;
...
//======[integration]============================================
#ifndef CENTER_QUADRATIC_STAGE
#define CENTER_QUADRATIC_STAGE

//M % 4 == 0, M >= 4
void __center_quadratic_stage(cudaStream_t stream,
	const float* X,
	float alpha, float beta, float gamma,
...
{
	if (M > 15) {
		if (N > 63) { center_quadratic4(stream, 3, 2, 3, 2, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride); return; }//[64, 16]
		if (N > 31) { center_quadratic4(stream, 3, 2, 2, 2, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride); return; }//[32, 16]
		if (N > 15) { center_quadratic4(stream, 3, 2, 1, 2, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride); return; }//[16, 16]
	}
	if (M > 7) {
		if (N > 127) { center_quadratic4(stream, 4, 1, 3, 2, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride); return; }//[ 64, 8]
		if (N >  63) { center_quadratic4(stream, 4, 1, 2, 2, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride); return; }//[ 32, 8]
		if (N >  31) { center_quadratic4(stream, 4, 1, 1, 2, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride); return; }//[ 16, 8]
	}
	center_quadratic4_small(stream, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride);
}

#endif


#ifndef CENTER_QUADRATIC
#define CENTER_QUADRATIC

//N = height, M = width
//(1) height = dim1, height % 4 != 0
//(2) width = dim0*dim2, width % 4 = 0
int __center_quadratic(cudaStream_t stream,
	const float* X,
	float alpha, float beta, float gamma,
	...
	int partNum)
{
	...
	if (nextN <= partNum) {//V.address = NULL, only 1 stage, write result to Y
		__center_quadratic_stage(stream, X, alpha, beta, gamma, dim1, dim2, N, M, Y, width, stride);
		return nextN;
	}

	//V.address != NULL, at least 2 stages
	__center_quadratic_stage(stream, X, alpha, beta, gamma, dim1, dim2, N, M, V, width, stride);
