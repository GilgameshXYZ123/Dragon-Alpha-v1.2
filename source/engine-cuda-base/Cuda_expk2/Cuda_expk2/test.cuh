...
//{
//	cout << "testCorrect:" << endl;
//	cout << "Xdim: " << dim0 << ',' << dim1 << ',' << dim2 << endl;
//
//	int length = dim0 * dim1 * dim2;
//	int height = dim0 * dim1;
//	int strideX = (dim2 + 3) >> 2 << 2;
//	int lengthXv = dim0 * dim1 * strideX;
//
//	float *X = newRandomFloatVec(height, dim2, strideX);
//
//	//exchange dims-------------------------------------------------------------
//	int dim[3]{ dim0, dim1, dim2 };
//	int t = dim[dimIndex1]; dim[dimIndex1] = dim[dimIndex2]; dim[dimIndex2] = t;
//	int Ydim0 = dim[0], Ydim1 = dim[1], Ydim2 = dim[2], Ydim3 = dim[3];
//	cout << "Ydim: " << Ydim0 << ',' << Ydim1 << ',' << Ydim2 << ',' << Ydim3 << endl;
//	int strideY = (Ydim3 + 3) >> 2 << 2;
...
void rot180_CPU(const float* X, int N, int IH, int IW, int IC, float* Y)
{
	for(int n=0; n<N; n++)
...
void testCorrect_rot180(int N, int IH, int IW, int IC)
{
	...
	__rot180(NULL, dX, dY, IH, IW, IC, length);
...
void test()
{
	//3, 4, 2, 3
	//12 * 8 = 96
	for (int dim1 = 1; dim1 <= 12; dim1++)
			for (int dim2 = 1; dim2 <= 32; dim2++)
				for (int dim3 = 1; dim3 <= 32; dim3++)
					testCorrect_transpose(10, dim1, dim2, dim3, 2, 1);

	//testCorrect_transpose(32, 32, 32, 32, 1, 2);
