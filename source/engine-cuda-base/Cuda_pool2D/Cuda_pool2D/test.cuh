...
float* newRandomFloatVec(int length)//0-256
{
	...
	for (int i = 0; i < length; i++) p[i] = (float)(rand() % 1000) / 1000 - 0.5f;
	return p;
}
	...
	for (int i = 0; i < GN; i++)
	{
		...
		for (int j = 0; j < GM; j++)
		{
			...
			for (int k = 0; k < GK; k++)
			{
				...
				if (ih < 0 || iw < 0 || ih >= IH || iw >= IW) { count++; continue; }
	...
	int sh, int sw, int ph, int pw)
{
	...
	//----------------------------------------------------------------
	//kmax4_W3(NULL, LBY, LBX, dX, IH, IW, dY, OH, OW, sh, sw, ph, pw, GN, GM, 0, 0);
	//kmax4_div2(NULL, LBY, LBX, dX, IH, IW, dY, OH, OW, GN, GM, 0, 0);
	...
	//kavg4_div2(NULL, LBY, LBX, dX, IH, IW, dY, OH, OW, GN, GM, 0, 0);
	//kavg4_W3(NULL, LBY, LBX, dX, IH, IW, dY, OH, OW, sh, sw, ph, pw, GN, GM, 0, 0);
...
void test()
{
	...
	//testCorrect<5, 0>(IH, IW, FH, FW, N, IC, sh, sw, ph, pw);
