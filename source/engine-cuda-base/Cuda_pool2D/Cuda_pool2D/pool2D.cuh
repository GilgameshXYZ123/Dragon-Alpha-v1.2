	...
	int ic_index, int j_index)
{
	...
	if (GN > 127) {//[128, 1]
		if (IS_DIV2) kmax4_div2(stream, 5, 0, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmax4_W3(stream, 5, 0, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmax4(stream, 5, 0, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dMaxBranch(127, 0); return;
	}
	if (GN > 63) {//[64, 2]
		if (IS_DIV2) kmax4_div2(stream, 4, 1, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmax4_W3(stream, 4, 1, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmax4(stream, 4, 1, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dMaxBranch(63, 1); return;
	}
	if (GN > 31) {//[32, 4]
		if (IS_DIV2) kmax4_div2(stream, 3, 2, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmax4_W3(stream, 3, 2, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmax4(stream, 3, 2, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dMaxBranch(31, 3); return;
	}
	if (GN > 15 && GM > 7) {//[16, 8]
		if (IS_DIV2) kmax4_div2(stream, 2, 3, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmax4_W3(stream, 2, 3, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmax4(stream, 2, 3, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
	...
	int ic_index, int j_index)
{
	...
	if (GN > 127) {//[128, 1]
		if (IS_DIV2) kmaxIdx4_div2(stream, 5, 0, X, IH, IW, Y, Index, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmaxIdx4_W3(stream, 5, 0, X, IH, IW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmaxIdx4(stream, 5, 0, X, IH, IW, FH, FW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dMaxIndexedBranch(127, 0); return;
	}
	if (GN > 63) {//[64, 2]
		if (IS_DIV2) kmaxIdx4_div2(stream, 4, 1, X, IH, IW, Y, Index, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmaxIdx4_W3(stream, 4, 1, X, IH, IW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmaxIdx4(stream, 4, 1, X, IH, IW, FH, FW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dMaxIndexedBranch(63, 1); return;
	}
	if (GN > 31) {//[32, 4]
		if (IS_DIV2) kmaxIdx4_div2(stream, 3, 2, X, IH, IW, Y, Index, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmaxIdx4_W3(stream, 3, 2, X, IH, IW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmaxIdx4(stream, 3, 2, X, IH, IW, FH, FW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dMaxIndexedBranch(31, 3); return;
	}
	if (GN > 15 && GM > 7) {//[16, 8]
		if (IS_DIV2) kmaxIdx4_div2(stream, 2, 3, X, IH, IW, Y, Index, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kmaxIdx4_W3(stream, 2, 3, X, IH, IW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kmaxIdx4(stream, 2, 3, X, IH, IW, FH, FW, Y, Index, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
	...
	int ic_index, int j_index)
{
	...
	if (GN > 127) {//[128, 1]
		if (IS_DIV2) kavg4_div2(stream, 5, 0, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kavg4_W3(stream, 5, 0, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kavg4(stream, 5, 0, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dAvgBranch(127, 0); return;
	}
	if (GN > 63) {//[64, 2]
		if (IS_DIV2) kavg4_div2(stream, 4, 1, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kavg4_W3(stream, 4, 1, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kavg4(stream, 4, 1, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dAvgBranch(63, 1); return;
	}
	if (GN > 31) {//[32, 4]
		if (IS_DIV2) kavg4_div2(stream, 3, 2, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kavg4_W3(stream, 3, 2, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kavg4(stream, 3, 2, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		poo2dAvgBranch(31, 3); return;
	}
	if (GN > 15 && GM > 7) {//[16, 8]
		if (IS_DIV2) kavg4_div2(stream, 2, 3, X, IH, IW, Y, OH, OW, GN, GM, ic_index, j_index);
		else if ((FH == 3) && (FW == 3)) kavg4_W3(stream, 2, 3, X, IH, IW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
		else kavg4(stream, 2, 3, X, IH, IW, FH, FW, Y, OH, OW, sh, sw, ph, pw, GN, GM, ic_index, j_index);
