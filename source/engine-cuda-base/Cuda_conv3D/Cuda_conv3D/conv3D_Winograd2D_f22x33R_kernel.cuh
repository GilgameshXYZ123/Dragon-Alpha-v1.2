...
//Common
#define conv3dWinograd2d_f22x33_k32x32R(stream, oc_index, j_index, D, DH, DW, G, Y, OH, OW, N, IC, OC)\
...
//(OC: 32, GM: 32 * 4), IC % 8 == 0, DW % 4 == 0
#define CONV_3D_WINOGRAD_2D_F22X33_KERNEL_32X32R_DW4
#ifndef CONV_3D_WINOGRAD_2D_F22X33_KERNEL_32X32R_DW4
#define CONV_3D_WINOGRAD_2D_F22X33_KERNEL_32X32R_DW4
...
//DW % 4 == 0
#define conv3dWinograd2d_f22x33_k32x32R_DW4(stream, oc_index, j_index, D, DH, DW, G, Y, OH, OW, N, IC, OC)\
	conv3dWinograd2d_f22x33_kernel_32x32R_DW4\
		<<< dim3((GN>>5), (GM>>5)), dim3(32, 8), 0, stream >>>\
			(D, (DH*DW), DW, G, Y, (OH*OW), OW, (N*DH*DW), IC, OC, oc_index, j_index)

__global__ void conv3dWinograd2d_f22x33_kernel_32x32R_DW4(
	const float* __restrict__ D, int DH_DW, int DW,
	const float* __restrict__ G,
	      float* __restrict__ Y, int OH_OW, int OW,
	int N_DH_DW, int IC, int OC,//sh = sw = 1 
	int oc_index, int j_index) 
{
	...
	//preload 1 group from X[N, IH, IW, IC]
	int X0 = doffset + ty * N_DH_DW;//[ic, n, ih, iw]
	X0 = X0 >> 2 << 2;
	const int X1 = X0 + DW, X2 = X0 + (DW << 1), X3 = X0 + DW * 3;
	*(float4*)(x    ) = *(float4*)(D + X0);//ih = 0
	*(float4*)(x + 2) = *(float4*)(D + X1);//ih = 1
	//*(float4*)(x + 4) = *(float4*)(D + X2);//ih = 2
	//*(float4*)(x + 6) = *(float4*)(D + X3);//ih = 3
	
	//x[0] = *(float2*)(D + X0); x[1] = *(float2*)(D + X0 + 2);
	//x[2] = *(float2*)(D + X1); x[3] = *(float2*)(D + X1 + 2);//ih = 1
	//x[4] = *(float2*)(D + X2); x[5] = *(float2*)(D + X2 + 2);//ih = 2
	//x[6] = *(float2*)(D + X3); x[7] = *(float2*)(D + X3 + 2);//ih = 3
...
#pragma unroll
		...
		//load 1 group from X[N, IH, IW, IC]
		int X0 = doffset + ic * N_DH_DW;//[ic, n, ih, iw]
		X0 = X0 >> 2 << 2;
		const int X1 = X0 + DW, X2 = X0 + (DW << 1), X3 = X0 + DW * 3;
		*(float4*)(x    ) = *(float4*)(D + X0);//ih = 0
		*(float4*)(x + 2) = *(float4*)(D + X1);//ih = 1
