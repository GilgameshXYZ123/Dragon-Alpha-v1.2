...
#ifndef CONV_3D_WINOGRAD_2D_F33X22R_KERNEL_H
#define CONV_3D_WINOGRAD_2D_F33X22R_KERNEL_H

//Winograd2D F(2*2, 3*3) convolution:
//(1) sh = sw = 1
//(2) FH = FW = 2
//(3) (OH, OW) % 3 == 0, the optimal cases
//
//Transformation
//(1) W[OC, 2, 2,  IC] -> G[IC, 3,  3, OC]
//(2) X[N, IH, IW, IC] -> D[IC, N, DH, DW]
//<1> DH = (OH - 1)*sh + FH = OH - 1 + 2 = OH + 1 = 3*x + 1
//<2> DW = (OW - 1)*sw + FW = OW - 1 + 2 = OW + 1 = 3*x + 1
//
//Formular:
//Y = A^T*[(G*W*G^T) <*> (B^T*X*B)]*A
//G = [1.0,  0.0]    B^T = [1,  0, -1, 0]    A^T = [1, 1,  1, 0]
//    [0.5,  0.5]          [0,  1,  1, 0]          [0, 1, -1, 0]
//    [0.5, -0.5]          [0, -1,  1, 0]          [0, 1,  1, 1]
//    [0.0,  1.0]          [0, -1,  0, 1]

#ifndef CONV_3D_WINOGRAD_2D_F33X22R_KERNEL_CALL
#define CONV_3D_WINOGRAD_2D_F33X22R_KERNEL_CALL

#define conv3dWinograd2d_f33x22_k32x32R(stream, oc_index, j_index, D, DH, DW, CW, Y, OH, OW, N, IC, OC)\
	conv3dWinograd2d_f33x22_kernel_32x32R\
		<<< dim3((GN>>5), (GM>>5)), dim3(32, 8), 0, stream >>>\
			(D, (DH*DW), DW, CW, Y, (OH*OW), OW, (N*DH*DW), IC, OC, oc_index, j_index)

#endif


//(OC: 32, GM: 32 * 4), IC % 8 == 0
#ifndef CONV_3D_WINOGRAD_2D_F33X22_KERNEL_32X32R
#define CONV_3D_WINOGRAD_2D_F33X22_KERNEL_32X32R
...
__global__ void conv3dWinograd2d_f33x22_kernel_32x32R(
	...
	int oc_index, int j_index) 
{
	...
	g[0] = *(float4*)(CW + G0);//[gt0: oc0 - oc3]
	g[1] = *(float4*)(CW + G1);//[gt1: oc0 - oc3]
	g[2] = *(float4*)(CW + G2);//[gt2: oc0 - oc3]
	g[3] = *(float4*)(CW + G3);//[gt3: oc0 - oc3]
