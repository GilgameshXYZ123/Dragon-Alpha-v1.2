...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC << 1 >> LB); oic < OIC; oic++)
		{
			...
			float d6 = (ly6 ? X[X6] : 0);
			float d7 = (ly7 ? X[X7] : 0);
			Ds[tx][ty] = winograd_f2x3_d(d0, d1, d2, d3);
			Ds[tx][ty + STEP2] = winograd_f2x3_d(d2, d3, d6, d7);
			...
			Gs[ty][tx] = winograd_f2x3_g(g0.x, g1.x, g2.x);
			Gs[ty][tx + STEP2] = winograd_f2x3_g(g0.y, g1.y, g2.y);
			Gs[ty][tx + STEP4] = winograd_f2x3_g(g0.z, g1.z, g2.z);
			Gs[ty][tx + STEP6] = winograd_f2x3_g(g0.w, g1.w, g2.w);
			__syncthreads();
...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC << 1 >> LB); oic < OIC; oic++)
		{
			...
			float d5 = (ly5 ? X[X5] : 0);
			Ds[tx][ty        ] = winograd_f2x3_d(d0, d1, d2, d3);
			Ds[tx][ty + STEP2] = winograd_f2x3_d(d2, d3, d4, d5);
	...
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC << 1 >> LB); oic < OIC; oic++)
		{
			...
			for (int ik = 0; ik < STEP; ik++)
			{
				...
				float4 d0 = winograd_f2x3_d(b0.x, b0.y, b0.z, b0.w);//d0, d1, d2, d3
				float4 d1 = winograd_f2x3_d(b0.z, b0.w, b1.x, b1.y);//d2, d3, d4, d5
				float4 d2 = winograd_f2x3_d(b2.x, b2.y, b2.z, b2.w);
				float4 d3 = winograd_f2x3_d(b2.z, b2.w, b3.x, b3.y);
...
//LB = 4: Size = 9, Time = 2.45823 msec, Performace = 7862.29 GFlop/s
template<int LB, int STEP, int STEP2, int STEP4, int STEP6>
...
//OH % 4 == 0, OW % 4 == 0
//(Y: BLOCK_SIZE*4, X: BLOCK_SIZE*4)
#ifndef CONV_3D_WINOGRAD_KERNEL_W3_D8
#define CONV_3D_WINOGRAD_KERNEL_W3_D8

#define wingrad_d8(stream, LB, oc_index, j_index, X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, GN, GM) \
	conv3dWinoGrad_kernel_d8<LB, (1<<LB>>1), (1<<LB), (2<<LB), (3<<LB), (1<<LB>>1)-1>\
		<<< dim3(GN>>LB>>3, GM>>LB>>3), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(X, IH, IW, CW, Y, (OH*OW), OW, IC, OC, ph, pw, oc_index, j_index)

//LB = 4: Size = 9, Time = 2.45823 msec, Performace = 7862.29 GFlop/s
template<int LB, int STEP, int STEP2, int STEP4, int STEP6, int STEP_m1>
__global__ void conv3dWinoGrad_kernel_d8(
	cudaTextureObject_t X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	float* __restrict__ Y, int OH_OW, int OW,
	int IC, int OC,
...
//OH % 4 == 0, OW % 4 == 0
//(Y: BLOCK_SIZE*4, X: BLOCK_SIZE*4)
#ifndef CONV_3D_WINOGRAD_KERNEL_W3_D9
#define CONV_3D_WINOGRAD_KERNEL_W3_D9

#define wingrad_d9(stream, LB, oc_index, j_index, X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, GN, GM) \
	conv3dWinoGrad_kernel_d9<LB, (1<<LB>>1), (1<<LB), (2<<LB), (3<<LB), (1<<LB>>1)-1>\
		<<< dim3(GN>>LB>>3, GM>>LB>>3), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(X, IH, IW, CW, Y, (OH*OW), OW, IC, OC, ph, pw, oc_index, j_index)

//LB = 4: Size = 9, Time = 2.45823 msec, Performace = 7862.29 GFlop/s
//LB = 3: Size = 9, Time = 2.22563 msec, Performace = 8684.01 GFlop/s
template<int LB, int STEP, int STEP2, int STEP4, int STEP6, int STEP_m1>
__global__ void conv3dWinoGrad_kernel_d9(
	cudaTextureObject_t X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	float* __restrict__ Y, int OH_OW, int OW,
	int IC, int OC,
...
#ifndef CONV_3D_WINOGRAD_KERNEL_W3_D10
#define CONV_3D_WINOGRAD_KERNEL_W3_D10

#define wingrad_d10(stream, LB, oc_index, j_index, X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, GN, GM) \
	conv3dWinoGrad_kernel_d10<LB, (1<<LB>>1), (1<<LB), (2<<LB), (3<<LB), (1<<LB>>1)-1>\
		<<< dim3(GN>>LB>>3, GM>>LB>>3), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(X, CW, Y, OH, OW, IC, OC, ph, pw, oc_index, j_index)

//LB = 4: Size = 9, Time = 2.45823 msec, Performace = 7862.29 GFlop/s
//LB = 3: Size = 9, Time = 2.2205 msec, Performace = 8704.07 GFlop/s
template<int LB, int STEP, int STEP2, int STEP4, int STEP6, int STEP_m1>
__global__ void conv3dWinoGrad_kernel_d10(
	...
	int oc_index, int j_index)
{
	...
	float4 v4 = F32_4_0, v5 = F32_4_0;
	float4 v6 = F32_4_0, v7 = F32_4_0;
	float4 v8 = F32_4_0, v9 = F32_4_0;
	float4 v10 = F32_4_0, v11 = F32_4_0;
	float4 v12 = F32_4_0, v13 = F32_4_0;
	float4 v14 = F32_4_0, v15 = F32_4_0;
	...
	for (int fh = 0; fh < 3; fh++, tih0++)
	{
		for (int oic = 0; oic < IC; oic += STEP)
		{
			...
			//load 2 group from X
			int xic = oic + (tx & STEP_m1);
			int X0 = ((tn0*OH + tih0)*OW + tiw0)*IC + xic;
			int X1 = X0 + IC, X2 = X1 + IC;
			int X3 = X2 + IC, X4 = X3 + IC, X5 = X4 + IC;

			bool lh0 = (tih0 >= 0) && (tih0 < OH);
			...
			float4 dsv0; float2 dsv1;
			dsv0.x = tex1Dfetch<float>(X, IF_int(ly0, X0, -1));
			dsv0.y = tex1Dfetch<float>(X, IF_int(ly1, X1, -1));
			dsv0.z = tex1Dfetch<float>(X, IF_int(ly2, X2, -1));
			dsv0.w = tex1Dfetch<float>(X, IF_int(ly3, X3, -1));
			dsv1.x = tex1Dfetch<float>(X, IF_int(ly4, X4, -1));
			dsv1.y = tex1Dfetch<float>(X, IF_int(ly5, X5, -1));
			Ds0[tx][ty] = dsv0; Ds1[tx][ty] = dsv1;
			__syncthreads();
...
#ifndef CONV_3D_WINOGRAD_KERNEL_W3_D11
#define CONV_3D_WINOGRAD_KERNEL_W3_D11

#define wingrad_d11(stream, LB, oc_index, j_index, X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, GN, GM) \
	conv3dWinoGrad_kernel_d11<LB, (1<<LB>>1), (1<<LB), (2<<LB), (3<<LB), (1<<LB>>1)-1>\
		<<< dim3(GN>>LB>>3, GM>>LB>>3), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(X, CW, Y, OH, OW, IC, OC, ph, pw, oc_index, j_index)
...
//LB = 3: Size = 9, Time = 2.2205 msec, Performace = 8704.07 GFlop/s
template<int LB, int STEP, int STEP2, int STEP4, int STEP6, int STEP_m1>
__global__ void conv3dWinoGrad_kernel_d11(
	cudaTextureObject_t X,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	float* __restrict__ Y, int OH, int OW,
	int IC, int OC,
...
#pragma unroll
			for (int ik = 0; ik < STEP; ik++)
			{
				...
				G_winograd4_W_V2(v0, v2, g0, g1, g2, g3, d0);
				G_winograd4_W_V2(v4, v6, g0, g1, g2, g3, d1);
				G_winograd4_W_V2(v8, v10, g0, g1, g2, g3, d2);
				G_winograd4_W_V2(v12, v14, g0, g1, g2, g3, d3);
				G_winograd4_W_V2(v1, v3, g4, g5, g6, g7, d0);
				G_winograd4_W_V2(v5, v7, g4, g5, g6, g7, d1);
				G_winograd4_W_V2(v9, v11, g4, g5, g6, g7, d2);
				G_winograd4_W_V2(v13, v15, g4, g5, g6, g7, d3);
			}
			__syncthreads();
		}
	}
