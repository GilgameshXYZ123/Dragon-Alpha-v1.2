...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC >> LB); oic < OIC; oic++)
		{
			...
			float g1 = W[W1];
			float g2 = W[W2];
			Gs[tx][ty] = winograd_f2x3_g(g0, g1, g2);
			__syncthreads();
...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC >> LB); oic < OIC; oic++)
		{
			...
			Gs[tx][(ty << 1)    ] = winograd_f2x3_g(g0.x, g1.x, g2.x);
			Gs[tx][(ty << 1) + 1] = winograd_f2x3_g(g0.y, g1.y, g2.y);
			__syncthreads();
...
#pragma unroll
			for (int ik = 0; ik < STEP; ik++) 
			{
				...
				//transposed 
				winograd_f2x3_add(v0, g0, d0);
				winograd_f2x3_add(v1, g1, d0);
			}
			__syncthreads();
		}
	}
...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC >> LB); oic < OIC; oic++)
		{
			...
			Ds[ty][(tx << 1)] = winograd_f2x3_d(d00, d01, d02, d03);
			...
			Ds[ty][(tx << 1) + 1] = winograd_f2x3_d(d20, d21, d22, d23);
