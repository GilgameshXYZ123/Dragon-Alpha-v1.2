...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC >> LB); oic < OIC; oic++)
		{
			...
			float d2 = (ly2 ? X[X2] : 0);
			float d3 = (ly3 ? X[X3] : 0);
			Ds[ty][tx] = winograd_f2x3_d(d0, d1, d2, d3);
			...
			Gs[tx][ty        ] = winograd_f2x3_g(g0.x, g1.x, g2.x);
			Gs[tx][ty + STEP ] = winograd_f2x3_g(g0.y, g1.y, g2.y);
			Gs[tx][ty + STEP2] = winograd_f2x3_g(g0.z, g1.z, g2.z);
			Gs[tx][ty + STEP3] = winograd_f2x3_g(g0.w, g1.w, g2.w);
			__syncthreads();
...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC >> LB); oic < OIC; oic++)
		{
			...
			Ds[ty][tx       ] = winograd_f2x3_d(d0, d1, d2, d3);
			Ds[ty][tx + STEP] = winograd_f2x3_d(d4, d5, d6, d7);
...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC >> LB); oic < OIC; oic++)
		{
			...
			Ds[tx][ty + STEP] = winograd_f2x3_d(d4, d5, d6, d7);
			...
			Gs[ty][tx + STEP] = winograd_f2x3_g(g0.y, g1.y, g2.y);

			Gs[ty][tx + STEP2] = winograd_f2x3_g(g0.z, g1.z, g2.z);
			Gs[ty][tx + STEP3] = winograd_f2x3_g(g0.w, g1.w, g2.w);
			__syncthreads();
...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC << 1 >> LB); oic < OIC; oic++)
		{
			...
			Gs[ty][(tx << 1)] = winograd_f2x3_g(g0.x, g1.x, g2.x);
			Gs[ty][(tx << 1) + 1] = winograd_f2x3_g(g0.y, g1.y, g2.y);
			__syncthreads();
...
#pragma unroll
	for (int fh = 0; fh < 3; fh++)
	{
		...
		for (int oic = 0, OIC = (IC << 1 >> LB); oic < OIC; oic++)
		{
			...
			float d7 = (ly7 ? X[X7] : 0);
			Ds[tx][ty        ] = winograd_f2x3_d(d0, d1, d2, d3);
			Ds[tx][ty + STEP2] = winograd_f2x3_d(d4, d5, d6, d7);
			__syncthreads();
