...
#ifndef WINOGRAD_F5X4_KERNEL1
#define WINOGRAD_F5X4_KERNEL1

//(N*OH*((OW + 4)/5))>>5
//64 * (32 * 5)
#define winograd_f5x4_k1(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, ph, pw) \
	winograd_f5x4_kernel1<FH>\
		<<< dim3(OC>>6, (N*OH*OW)/160), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, oc_index, j_index)

//Size = 25, Time = 3.03144 msec, Performace = 17710.1 GFlop/s
template<int FH>
__global__ void winograd_f5x4_kernel1(
	...
	int oc_index, int j_index)
{
	...
	const int bj0 = (blockIdx.y * 160) + j_index;//32 group of Y
	const int Ds_k = (tx & 7), Ds_i = (ty << 1) + (tx > 7);//[8, 32]
	const int tj0 = bj0  + Ds_i * 5;
	const int OW5 = (OW + 4) / 5 * 5, OH_OW5 = OH * OW5;
	int tn0, toh0, tow0; {
		tn0 = tj0 / OH_OW5; int jr = tj0 - tn0 * OH_OW5; 
		toh0 = jr / OW5; tow0 = jr - toh0 * OW5;
	}
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		const int W0 = fh * 4 * IC * OC;//with the same tx
...
#pragma unroll
			...
			const int W0 = (fh * 4 * IC + oic) * OC;//fh, ic, fw, oc
			const int W1 = W0 + IC * OC;
			const int W2 = W0 + (IC * OC << 1);
			const int W3 = W0 + IC * OC * 3;
			float2 w0 = *(float2*)(CW + W0);
...
#pragma unroll
	...
	*(float4*)(Y + Y00 + OC * 20) = { y0[0], y1[0], y2[0], y3[0] };
	*(float4*)(Y + Y00 + OC * 21) = { y0[1], y1[1], y2[1], y3[1] };
	*(float4*)(Y + Y00 + OC * 22) = { y0[2], y1[2], y2[2], y3[2] };
	*(float4*)(Y + Y00 + OC * 23) = { y0[3], y1[3], y2[3], y3[3] };
	*(float4*)(Y + Y00 + OC * 24) = { y0[4], y1[4], y2[4], y3[4] };
}

#endif


#ifndef WINOGRAD_F5X4_KERNEL2
#define WINOGRAD_F5X4_KERNEL2

#define winograd_f5x4_k2(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, ph, pw) \
	winograd_f5x4_kernel2<FH>\
		<<< dim3(OC>>6, (N*OH*((OW + 4)/5))>>5), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, oc_index, j_index)

//Size = 25, Time = 2.99952 msec, Performace = 17898.6 GFlop/s
//Size = 25, Time = 2.79652 msec, Performace = 19197.8 GFlop/s

//Size = 50, Time = 5.87437 msec, Performace = 18278.4 GFlop/s
//Size = 50, Time = 5.42794 msec, Performace = 19781.8 GFlop/s

template<int FH>
__global__ void winograd_f5x4_kernel2(
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH, int OW,
	int IC, int OC,
...
//ph = pw = 2
#ifndef WINOGRAD_F5X4_KERNEL3
#define WINOGRAD_F5X4_KERNEL3

#define winograd_f5x4_k3(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC) \
	winograd_f5x4_kernel3<FH>\
		<<< dim3(OC>>6, (N*OH*((OW + 4)/5))>>5), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, oc_index, j_index)

//Size = 25, Time = 3.03144 msec, Performace = 17710.1 GFlop/s
template<int FH>
__global__ void winograd_f5x4_kernel3(
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH, int OW,
	int IC, int OC,
	...
	int oc_index, int j_index)
{
	...
	const int tih0 = toh0 - 2, tiw0 = tow0 - 2;
	const int X0 = ((tn0*IH + tih0)*IW + tiw0)*IC + Ds_k;//X[tn0, tih0, tiw0, Ds_k]
