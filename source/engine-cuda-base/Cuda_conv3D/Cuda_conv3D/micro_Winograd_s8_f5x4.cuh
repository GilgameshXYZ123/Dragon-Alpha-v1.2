...
//FW = 4
//group = 5 elements
#ifndef MICRO_WINOGRAD_F5X4_H
#define MICRO_WINOGRAD_F5X4_H
...
//[ 1/90    1/45    2/45    4/45]
//[ 1/90   -1/45    2/45   -4/45]
//[ 32/45   16/45   8/45    4/45]
//[ 32/45  -16/45   8/45   -4/45]
//[   0       0       0        1]
#define winograd_f5x4_g(g, w0, w1, w2, w3) {\
	g[0] = w0;\
	g[1] = -0.22222222f * (w0 + w1 + w2 + w3);\
	g[2] = -0.22222222f * (w0 - w1 + w2 - w3);\
	g[3] = 0.011111111f*w0 + 0.022222222f*w1 + 0.044444444f*w2 + 0.088888889f*w3;\
	g[4] = 0.011111111f*w0 - 0.022222222f*w1 + 0.044444444f*w2 - 0.088888889f*w3;\
	g[5] = 0.71111111f *w0 + 0.35555556f *w1 + 0.17777778f *w2 + 0.088888889f*w3;\
	g[6] = 0.71111111f *w0 - 0.35555556f *w1 + 0.17777778f *w2 - 0.088888889f*w3;\
	g[7] = w3; }
...
#define winograd_f5x4_G(g, w0, w1, w2, w3) {\
	float t1, t2;\
	g[0] = w0;\
	t1 = -0.22222222f*(w0 + w2);\
	t2 = -0.22222222f*(w1 + w3);\
	g[1] = t1 + t2; g[2] = t1 - t2;\
	t1 = 0.011111111f*w0 + 0.044444444f*w2;\
	t2 = 0.022222222f*w1 + 0.088888889f*w3;\
	g[3] = t1 + t2; g[4] = t1 - t2;\
	t1 = 0.71111111f *w0 + 0.17777778f *w2;\
	t2 = 0.35555556f *w1 + 0.088888889f*w3;\
	g[5] = t1 + t2; g[6] = t1 - t2; \
	g[7] = w3; }
...
//[0  1   1   4    4   1/4    1/4    0]
//[0  1  -1   8   -8   1/8   -1/8   0]
//[0  1   1   16  16   1/16   1/16  1]
#define winograd_f5x4_y(y, a) {\
	...
	y[5] =        a[1] + a[2] + 16.0f*a[3] + 16.0f*a[4] + 0.0625f*a[5] + 0.0625f*a[6] + a[7]; }
...
#define winograd_f5x4_Y(y, a) {\
	...
	y[5] =        t0 + 16.0f * t2 + 0.0625f*t4 + a[7]; }

//optimized: with less multiplications
//for vector type
#define winograd_f5x4_Y_vec(y, a, elem) {\
	...
	y[2] =             t0 +  4.0f *t2 + 0.25f  *t4;\
	y[3] =             t1 +  8.0f *t3 + 0.125f *t5;\
	y[4] =             t0 + 16.0f *t2 + 0.0625f*t4 + a[7].elem; }
