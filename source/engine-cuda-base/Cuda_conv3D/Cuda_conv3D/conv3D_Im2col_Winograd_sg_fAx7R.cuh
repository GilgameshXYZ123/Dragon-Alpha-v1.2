...
#ifndef CONV_3D_WINOGRAD_S16_F10X7R_H
#define CONV_3D_WINOGRAD_S16_F10X7R_H

//(1) sh = sw = 1
//(2) FW = 7
//(3) OW % 10: group = 10 elements
//(4) Remode the kernel: W[OC, FH, FW, IC] -> CW[FH, FW, IC, OC]
#ifndef CONV_3D_WINOGRAD_F10X7R_CALL
#define CONV_3D_WINOGRAD_F10X7R_CALL

//================[Standard: 32(OC) * 320(GM)]===============================
//pw <= 3
#define conv3dWinograd_f10x7_k32x320R_p3_tex(stream, X, IH, IW, CW, FH, Y, OH, OW, N, IC, OC, ph, pw) \
	conv3dWinograd_f10x7_kernel_32x320R_p3_tex<FH>\
		<<< dim3(OC>>5, ((N*OH)>>5) * (OW/10)), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw)

//pw <= 3, template<IC>
#define conv3dWinograd_f10x7_k32x320R_p3_CT_tex(stream, X, IH, IW, CW, FH, Y, OH, OW, N, IC, OC, ph, pw) \
	conv3dWinograd_f10x7_kernel_32x320R_p3_CT_tex<FH, IC, OC>\
		<<< dim3(OC>>5, ((N*OH)>>5) * (OW/10)), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, ph, pw)

//================[Standard: 64(OC) * 320(GM)]===============================
#define conv3dWinograd_f10x7_k64x320R_p3_tex(stream, X, IH, IW, CW, FH, Y, OH, OW, N, IC, OC, ph, pw) \
	conv3dWinograd_f10x7_kernel_64x320R_p3_tex<FH>\
		<<< dim3(OC>>6, ((N*OH)>>5) * (OW/10)), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw)
...
//================[Standard: 32(OC) * 320(GM)]===============================
//LB = 4, IC % 8 == 0, pw <= 3
#ifndef CONV_3D_WINOGRAD_F10X7_32X320R_P3_TEXTURE
#define CONV_3D_WINOGRAD_F10X7_32X320R_P3_TEXTURE

//for: Feature = (80, 80), [N, IC, OC] = [64, 64, 64]
//LB = 4: Size = 76.5625, Time = 7.08423 msec, Performace = 23208.8 GFlop/s
//WB = 4: Size = 76.5625, Time = 6.66337 msec, Performace = 24674.7 GFlop/s
//for: Feature = (40, 40), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 76.5625, Time = 8.18665 msec, Performace = 20083.5 GFlop/s
//WB = 4: Size = 76.5625, Time = 7.82091 msec, Performace = 21022.7 GFlop/s
//for: Feature = (20, 20), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 76.5625, Time = 8.3416  msec, Performace = 19710.5 GFlop/s
//WB = 4: Size = 76.5625, Time = 8.06029 msec, Performace = 20398.4 GFlop/s

template<int FH>
__global__ void conv3dWinograd_f10x7_kernel_32x320R_p3_tex(
	...
	int ph, int pw)//sh = sw = 1
{
	...
	const int OW10 = (OW / 10) * 10, OH_OW10 = OH * OW10;
	get_n_oh_ow_Temp(tj0, tn0, toh0, tow0, OH_OW10, OW10);
	...
	const int yj0 = bj0 + (DIdx + (ux >> 1)) * 10;
	get_n_oh_ow_Temp(yj0, yn0, yoh0, yow0, OH_OW10, OW10);
	const int Y00 = ((yn0*OH + yoh0)*OW + yow0)*OC + yoc0;

	//======[compute area1: local]======================================================
	float w[7], g[16], x[16], d[16];
	for (int fh = 0; fh < FH; fh++) {
		//load 1 group from CW[FH, 7, IC, OC]
		...
		bool lyD = lh0 && (tiw0 >= -13) && (tiw0 + 13 < IW);//pw = +1
		bool lyE = lh0 && (tiw0 >= -14) && (tiw0 + 14 < IW);//pw = +2
		bool lyF = lh0 && (tiw0 >= -15) && (tiw0 + 15 < IW);//pw = +3
		int tX0 = X0 + fh * IW * IC;
		...
		tXB = IF_int(lh0, tXB, -1); tXC = IF_int(lh0, tXC, -1); 
		tXD = IF_int(lyD, tXD, -1);
		...
		for (int oic = 8; oic < IC; oic += 8) {
			//Winograd Transform: W(7) -> G(16), X(16) -> D(16)
...
#pragma unroll
	...
	float a[16], y0[10], y1[10], y2[10], y3[10];//oc0, oc1, oc2, oc3
	__syncthreads();
	...
	*(float4*)(Y + Y00 + OC *  9) = float4{ y0[9], y1[9], y2[9], y3[9] };
}

#endif


//LB = 4, IC % 8 == 0,pw <= 3, template<IC>
#ifndef CONV_3D_WINOGRAD_F10X7_32X320R_P3_CT_TEXTURE
#define CONV_3D_WINOGRAD_F10X7_32X320R_P3_CT_TEXTURE

//for: Feature = (120, 120), [N, IC, OC] = [32, 64, 64]
//LB = 4: Size = 86.1328, Time = 7.34329 msec, Performace = 25188.8 GFlop/s
//WB = 4: Size = 86.1328, Time = 6.92344 msec, Performace = 26716.3 GFlop/s
//cuDNN-NCHW-GEMM-implicit-prec: Size = 86.132813, Time = 11.801 msec, Performance = 15673.994141 GFlop/s
//cuDNN-NHWC-GEMM-implicit-prec: Size = 86.132813, Time = 11.084 msec, Performance = 16687.910156 GFlop/s
//for: Feature = (80, 80), [N, IC, OC] = [64, 64, 64]
...
__global__ void conv3dWinograd_f10x7_kernel_32x320R_p3_CT_tex(
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH, int OW,
	int ph, int pw)//sh = sw = 1
...
//================[Standard: 64(OC) * 320(GM)]===============================
//LB = 4, IC % 8 == 0, pw <= 3
#ifndef CONV_3D_WINOGRAD_F10X7_64X320R_P3_TEXTURE
#define CONV_3D_WINOGRAD_F10X7_64X320R_P3_TEXTURE

//for: Feature = (80, 80), [N, IC, OC] = [64, 64, 64]
//LB = 4: Size = 76.5625, Time = 5.88522 msec, Performace = 27937.2 GFlop/s
//WB = 4: Size = 76.5625, Time = 5.4201  msec, Performace = 30334.6 GFlop/s
//for: Feature = (40, 40), [N, IC, OC] = [64, 128, 128]
//LB = 4: Size = 76.5625, Time = 6.32423 msec, Performace = 25997.9 GFlop/s
//WB = 4: Size = 76.5625, Time = 5.83215 msec, Performace = 28191.5 GFlop/s
//for: Feature = (20, 20), [N, IC, OC] = [64, 256, 256]
//LB = 4: Size = 76.5625, Time = 6.08338 msec, Performace = 27027.2 GFlop/s
//WB = 4: Size = 76.5625, Time = 5.65674 msec, Performace = 29065.6 GFlop/s
//for: Feature = (10, 10), [N, IC, OC] = [64, 512, 512]
//LB = 4: Size = 76.5625, Time = 6.35261 msec, Performace = 25881.8 GFlop/s
//WB = 4: Size = 76.5625, Time = 5.83456 msec, Performace = 28179.8 GFlop/s

template<int FH>
__global__ void conv3dWinograd_f10x7_kernel_64x320R_p3_tex(
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH, int OW,
	int IC, int OC,
	int ph, int pw)//sh = sw = 1
{
	...
	//prepare for X[N, IH, IW, IC]
	const int bj0 = by * 320;//32 * 8 = 256
	const int Xk = (tx & 7), Xi = (ty << 1) + (tx > 7);//[8, 32]
	const int tj0 = bj0 + Xi * 10;//OW % 8
	const int OW10 = (OW / 10) * 10, OH_OW10 = OH * OW10;
	get_n_oh_ow_Temp(tj0, tn0, toh0, tow0, OH_OW10, OW10);
	const int tih0 = toh0 - ph, tiw0 = tow0 - pw;
	const int X0 = ((tn0*IH + tih0)*IW + tiw0)*IC + Xk;//X[tn0, tih0, tiw0, Xk]
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		tX9 = IF_int(lh0, tX9, -1); tXA = IF_int(lh0, tXA, -1);
		tXB = IF_int(lh0, tXB, -1); tXC = IF_int(lh0, tXC, -1); 
		tXD = IF_int(lyD, tXD, -1); tXE = IF_int(lyE, tXE, -1); tXF = IF_int(lyF, tXF, -1);
		x[ 0] = tex1Dfetch<float>(X, tX0); x[ 1] = tex1Dfetch<float>(X, tX1);
		x[ 2] = tex1Dfetch<float>(X, tX2); x[ 3] = tex1Dfetch<float>(X, tX3);
		x[ 4] = tex1Dfetch<float>(X, tX4); x[ 5] = tex1Dfetch<float>(X, tX5);
		x[ 6] = tex1Dfetch<float>(X, tX6); x[ 7] = tex1Dfetch<float>(X, tX7);
...
#pragma unroll
	...
	*(float4*)(Y + Y00 + OC * 9 + 8) = float4{ y0[9], y1[9], y2[9], y3[9] };
}
