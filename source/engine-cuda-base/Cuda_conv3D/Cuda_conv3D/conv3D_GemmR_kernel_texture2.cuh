...
#define conv3dGemm_k88R4W3_ic2pow_tex(stream, LB, oc_index, j_index, X, IH, IW, CW, Y, OH, OW, LIC, OC, sh, sw, ph, pw, GN, GM) \
	conv3dGemm_kernel_8_8R4W3_IC2pow_texture<LB, (1<<LB>>1), (1<<LB>>1)-1>\
		<<< dim3(GN>>LB>>3, GM>>LB>>3), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(X,IH,IW, CW, Y,(OH*OW),OW, LIC,OC, sh,sw,ph,pw,\
			oc_index, j_index)
...
//=====[FH = FW = 5]==========================================
#define conv3dGemm_k88R4W7_ic2pow_tex(stream, LB, oc_index, j_index, X, IH, IW, CW, Y, OH, OW, LIC, OC, sh, sw, ph, pw, GN, GM) \
	conv3dGemm_kernel_8_8R4W7_IC2pow_texture<LB, (1<<LB>>1), (1<<LB>>1)-1>\
		<<< dim3(GN>>LB>>3, GM>>LB>>3), dim3(1<<LB, 1<<LB), 0, stream >>>\
			(X,IH,IW, CW, Y,(OH*OW),OW, LIC,OC, sh,sw,ph,pw,\
			oc_index, j_index)
...
//feature: (256, 128, 128, 3), FH = FW = 3x3
//LB = 4: Size = 9, Time = 2.89967 msec, Performace = 6665.36 GFlop/s
	...
	int oc_index, int j_index)
{
	...
	//load 4 elements from X[N, IH, IW, IC]
	const int X_k = (tx & STEP_m1);
	int Idx = X_k >> LIC; char fhw = XIDX_W3[Idx];
	int X_fh = fhw >> 2, X_fw = fhw & 3; 
	int xoffset = X1 + ((X_fh * IW + X_fw - Idx) << LIC) + X_k;
	Xs[0][tx][ty] = SaveX4x_tex(X, X_fh, X_fw, IH, IW, xoffset, sw_IC,
		toh0, tow0, tow1, tow2, tow3);

	//load 4 elements from W[OC, FH, FW, IC]
	const int W_k = (ty & STEP_m1);
	Ws[0][ty][tx] = *(float4*)(CW + W_k * OC);
	__syncthreads();

	for (int ok = STEP; ok < GK; ok += STEP)//OK = GK / STEP
	{
#pragma unroll
		for (int ik = 0; ik < STEP; ik++)
		{
...
//(Y: BLOCK_SIZE*8, X: BLOCK_SIZE*8), GK % (BLOCK_SIZE/2) == 0, IC is power of 2
//LB = 4, GK % 8 == 0 
#ifndef CONV_3D_GEMM_KERNEL_8_8R4W7_IC_2POW_TEXTURE
#define CONV_3D_GEMM_KERNEL_8_8R4W7_IC_2POW_TEXTURE

//[IH, IW] = [112, 122] -> [OH, OW] = [56, 56], [FH, FW] = 7, [N, IC, OC] = [32, 32, 128]
//LB = 4: Size = 18.7578, Time = 3.80289 msec, Performace = 10592.5 GFlop/s
//[IH, IW] = [112, 122] -> [OH, OW] = [56, 56], [FH, FW] = 7, [N, IC, OC] = [128, 4, 64]
//LB = 3: Size = 4.68945, Time = 1.06255 msec, Performace = 9477.7 GFlop/s
template<int LB, int STEP, int STEP_m1>
__global__ void conv3dGemm_kernel_8_8R4W7_IC2pow_texture(
	cudaTextureObject_t X, int IH, int IW,
	const float* __restrict__ CW,
	float* __restrict__ Y, int OH_OW, int OW,
	int LIC, int OC,
	...
	int oc_index, int j_index)
{
	...
	toh0 = toh0 * sh - ph, tow0 = tow0 * sw - pw;
	int tow1 = tow0 + sw, tow2 = tow1 + sw, tow3 = tow2 + sw;
	const int X1 = ((tn0*IH + toh0)*IW + tow1) << LIC;
	const int sw_IC = sw << LIC;
	...
	const int xoffset = X1 + ((X_fh * IW + X_fw - Idx) << LIC) + X_k;
	Xs[buf][tx][ty] = SaveX4x_tex(X, X_fh, X_fw, IH, IW, xoffset, sw_IC,
		toh0, tow0, tow1, tow2, tow3);
