...
#ifndef WINOGRAD_6X3_KERNEL1
#define WINOGRAD_6X3_KERNEL1

//Size = 81, Time = 11.1483 msec, Performace = 15602.9 GFlop/s

#define winograd_6x3_k1(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, ph, pw) \
	winograd_6x3_kernel1<FH>\
		<<< dim3(OC>>6, (N*OH*((OW + 2)/3))>>5), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, oc_index, j_index)

template<int FH>
__global__ void winograd_6x3_kernel1(
	...
	int oc_index, int j_index)
{
	...
	const int bj0 = (blockIdx.y * 96) + j_index;//32 group of Y
	const int Ds_k = (tx & 7), Ds_i = (ty << 1) + (tx > 7);//[8, 32]
	const int tj0 = bj0  + Ds_i * 3;
	const int OW3 = (OW + 2) / 3 * 3, OH_OW3 = OH * OW3;
	int tn0, toh0, tow0; {
		tn0 = tj0 / OH_OW3; int jr = tj0 - tn0 * OH_OW3; 
		toh0 = jr / OW3; tow0 = jr - toh0 * OW3;
	}
	...
	for (int fh = 0; fh < FH; fh++) {
		...
		const int W0 = fh * 6 * IC * OC;//with the same tx
...
#pragma unroll
			...
			const int W0 = (fh * 6 * IC + oic) * OC;//fh, ic, fw, oc
			const int W1 = W0 + IC * OC, W2 = W0 + (IC * OC << 1);
			const int W3 = W0 + IC * OC * 3, W4 = W0 + (IC * OC << 2);
			const int W5 = W0 + IC * OC * 5;
			float2 w0 = *(float2*)(CW + W0);
...
#pragma unroll
	...
	*(float4*)(Y + Y00 + OC * 12) = { y0[0], y1[0], y2[0], y3[0] };
	*(float4*)(Y + Y00 + OC * 13) = { y0[1], y1[1], y2[1], y3[1] };
	*(float4*)(Y + Y00 + OC * 14) = { y0[2], y1[2], y2[2], y3[2] };
}
...
//Standard
#ifndef WINOGRAD_6X3_KERNEL2
#define WINOGRAD_6X3_KERNEL2

//Size = 40.5, Time = 5.56871 msec, Performace = 15618.2 GFlop/s
//Size = 162, Time = 22.1401 msec, Performace = 15713.2 GFlop/s

//WB: Size = 40.5, Time = 5.53422 msec, Performace = 15715.5 GFlop/s

#define winograd_6x3_k2(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, ph, pw) \
	winograd_6x3_kernel2<FH>\
		<<< dim3(OC>>6, (N*OH*((OW + 2)/3))>>5), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, oc_index, j_index)

template<int FH>
__global__ void winograd_6x3_kernel2(
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH, int OW,
	int IC, int OC,
...
//[FH, IC,  FW, OC]
#ifndef WINOGRAD_6X3_KERNEL3
#define WINOGRAD_6X3_KERNEL3

//Size = 40.5, Time = 5.32085 msec, Performace = 16345.7 GFlop/s
//Size = 40.5, Time = 5.09489 msec, Performace = 17070.6 GFlop/s

#define winograd_6x3_k3(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, ph, pw) \
	winograd_6x3_kernel3<FH>\
		<<< dim3(OC>>6, (N*OH*((OW + 2)/3))>>5), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, OH, OW, IC, OC, ph, pw, oc_index, j_index)

template<int FH>
__global__ void winograd_6x3_kernel3(
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH, int OW,
	int IC, int OC,
	...
	int oc_index, int j_index)
{
	...
	const int Gs_k = (ty & 7), Gs_i = ((tx << 1) + (ty > 7)) << 1;//[8, 64]
	const int toc0 = boc0 + Gs_i;
	CW += Gs_k * OC * 6 + toc0;//CW[0, Gs_k, 0, toc0]
