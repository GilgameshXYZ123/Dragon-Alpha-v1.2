...
//64 * 32(4 elements) group
//GM = N*OH*OW
//OW % 4 == 0
#define WG_f4x5_K1(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, ph, pw, GN, GM) \
	WG_f4x5_Kernel1<FH>\
		<<< dim3(GN>>6, GM>>7), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, (OH*OW), OW, IC, OC, ph, pw, oc_index, j_index)

//Size = 25, Time = 3.01301 msec, Performace = 17818.4 GFlop/s

template<int FH>
__global__ void WG_f4x5_Kernel1(  
	...
	int oc_index, int j_index)
{
	...
	const int bj0 = (blockIdx.y << 7) + j_index;//32 group of Y
	const int Ds_k = (tx & 7), Ds_i = (ty << 1) + (tx > 7);//[8, 32]
	const int tj0 = bj0  + (Ds_i << 2);
...
#pragma unroll
	...
	const int Y10 = Y00 + 16 * OC;//ux: j4 -> j7
	__syncthreads();
	...
	*(float4*)(&Ys[ux][uy][6]) = { v6.x, v6.y, v7.x, v7.y };
	...
	{
		float ta[8] = { a[0].x, a[1].x, a[2].x, a[3].x, a[4].x, a[5].x, a[6].x, a[7].x };
		winograd_f4x5_y(y0, ta);//{oc0 / oc4} * { j0 - j3 }
	}

	{
		float ta[8] = { a[0].y, a[1].y, a[2].y, a[3].y, a[4].y, a[5].y, a[6].y, a[7].y };
		winograd_f4x5_y(y1, ta);//{oc1 / oc5} * { j0 - j3 }
	}
	__syncthreads();
	...
	*(float4*)(&Ys[ux][uy][6]) = { v6.z, v6.w, v7.z, v7.w };
	...
	{
		float ta[8] = { a[0].x, a[1].x, a[2].x, a[3].x, a[4].x, a[5].x, a[6].x, a[7].x };
		winograd_f4x5_y(y2, ta);//{oc0 / oc4} * { j0 - j3 }
	}

	{
		float ta[8] = { a[0].y, a[1].y, a[2].y, a[3].y, a[4].y, a[5].y, a[6].y, a[7].y };
		winograd_f4x5_y(y3, ta);//{oc1 / oc5} * { j0 - j3 }
	}
	...
	*(float4*)(&Ys[ux][uy][6]) = { v14.x, v14.y, v15.x, v15.y };
	...
	{
		float ta[8] = { a[0].x, a[1].x, a[2].x, a[3].x, a[4].x, a[5].x, a[6].x, a[7].x };
		winograd_f4x5_y(y0, ta);//{oc0 / oc4} * { j4 - j7 }
	}

	{
		float ta[8] = { a[0].y, a[1].y, a[2].y, a[3].y, a[4].y, a[5].y, a[6].y, a[7].y };
		winograd_f4x5_y(y1, ta);//{oc1 / oc5} * { j4 - j7 }
	}
	__syncthreads();
	...
	*(float4*)(&Ys[ux][uy][6]) = { v14.z, v14.w, v15.z, v15.w };
	...
	{
		float ta[8] = { a[0].x, a[1].x, a[2].x, a[3].x, a[4].x, a[5].x, a[6].x, a[7].x };
		winograd_f4x5_y(y2, ta);//{oc2 / oc6} * { j4 - j7 }
	}

	{
		float ta[8] = { a[0].y, a[1].y, a[2].y, a[3].y, a[4].y, a[5].y, a[6].y, a[7].y };
		winograd_f4x5_y(y3, ta);//{oc3 / oc7} * { j4 - j7 }
	}
...
//GM = N*OH*OW
//OW % 4 == 0
#define WG_f4x5_K2(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, ph, pw, GN, GM) \
	WG_f4x5_Kernel2<FH>\
		<<< dim3(GN>>6, GM>>7), dim3(16, 16), 0, stream>>>\
			(X, IH, IW, CW, Y, (OH*OW), OW, IC, OC, ph, pw, oc_index, j_index)

//Size = 25, Time = 3.01301 msec, Performace = 17818.4 GFlop/s
//Size = 25, Time = 2.8426 msec, Performace = 18886.6 GFlop/s
//Size = 50, Time = 5.9554 msec, Performace = 18029.7 GFlop/s
//Size = 50, Time = 5.50396 msec, Performace = 19508.5 GFlop/s 

template<int FH>
__global__ void WG_f4x5_Kernel2(  
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH_OW, int OW,
	int IC, int OC,
...
#pragma unroll
	...
	*(float4*)(Y + Y00 + OC * 16 ) = { y0[0], y1[0], y2[0], y3[0] };
	*(float4*)(Y + Y00 + OC * 17) = { y0[1], y1[1], y2[1], y3[1] };
	*(float4*)(Y + Y00 + OC * 18) = { y0[2], y1[2], y2[2], y3[2] };
	*(float4*)(Y + Y00 + OC * 19) = { y0[3], y1[3], y2[3], y3[3] };
}
...
//GM = N*OH*OW
//OW % 4 == 0
#define WG_f4x5_K3(stream, oc_index, j_index, X, IH, IW, CW, FH, Y, OH, OW, IC, OC, GN, GM) \
	WG_f4x5_Kernel3<FH>\
		<<< dim3(GN>>6, GM>>7), dim3(16, 16), 0, stream>>>\
...
//Size = 25, Time = 3.01301 msec, Performace = 17818.4 GFlop/s
//Size = 25, Time = 2.8426 msec, Performace = 18886.6 GFlop/s
//Size = 50, Time = 5.72111 msec, Performace = 18768.1 GFlop/s
//Size = 50, Time = 5.33394 msec, Performace = 20130.3 GFlop/s

template<int FH>
__global__ void WG_f4x5_Kernel3(  
	cudaTextureObject_t        X, int IH, int IW,
	const float* __restrict__ CW,//[FH, FW, IC, OC]
	      float* __restrict__  Y, int OH_OW, int OW,
	int IC, int OC,//ph = pw = 2, sh = sw = 1
	int oc_index, int j_index)
{
	const int tx = threadIdx.x, ty = threadIdx.y;
