...
#define C3D_WG2D_KERNEL_C1
#ifndef C3D_WG2D_KERNEL_C1 
#define C3D_WG2D_KERNEL_C1

#define c3d_wg2d_c1(stream, oc_index, j_index, D, DH, DW, G, Y, OH, OW, N, IC, OC)\
	c3d_wg2d_kernel_C1\
		<<< dim3((GN>>5), (GM>>5)), dim3(32, 8), 0, stream >>>\
			(D, (DH*DW), DW, G, Y, OH, OW, N, IC, OC, oc_index, j_index)\

//Size = 9, Time = 1.23147 msec, Performace = 15694.5 GFlop/s
//Size = 9, Time = 1.21633 msec, Performace = 15889.8 GFlop/s
//Size = 9, Time = 1.00934 msec, Performace = 19148.6 GFlop/s
//template<int OC = 128>  
__global__ void c3d_wg2d_kernel_C1(
...
#define C3D_WG2D_KERNEL_C2
#ifndef C3D_WG2D_KERNEL_C2 
#define C3D_WG2D_KERNEL_C2

#define c3d_wg2d_c2(stream, oc_index, j_index, D, DH, DW, G, Y, OH, OW, N, IC, OC)\
	c3d_wg2d_kernel_C2\
		<<< dim3((GN>>5), (GM>>5)), dim3(32, 8), 0, stream >>>\
			(D, (DH*DW), DW, G, Y, (OH*OW), OW, (N*DH*DW), IC, OC, oc_index, j_index)\

//Size = 36, Time = 5.19619 msec, Performace = 14878.1 GFlop/s

//Size = 9, Time = 1.23147 msec, Performace = 15694.5 GFlop/s
//Size = 9, Time = 1.21633 msec, Performace = 15889.8 GFlop/s
//Size = 9, Time = 1.00934 msec, Performace = 19148.6 GFlop/s
//template<int OC = 128>  
__global__ void c3d_wg2d_kernel_C2(
	const float* __restrict__ D, int DH_DW, int DW,
	const float* __restrict__ G,
	      float* __restrict__ Y, int OH_OW, int OW,
	int GM, int IC, int OC,//sh = sw = 1 
	int oc_index, int j_index) 
{
	const int tx = threadIdx.x, ty = threadIdx.y;

	__shared__ float Gs[10][16][36];
	__shared__ float Ds[10][16][36];
	...
	//preload 1 group from X[N, IH, IW, IC]
	const int X0 = doffset + ty * GM;//[ic, n, ih, iw]
	const int X1 = X0 + DW, X2 = X0 + (DW << 1), X3 = X0 + DW * 3;
	x[0] = *(float2*)(D + X0); x[1] = *(float2*)(D + X0 + 2);//ih = 0
	x[2] = *(float2*)(D + X1); x[3] = *(float2*)(D + X1 + 2);//ih = 1
	x[4] = *(float2*)(D + X2); x[5] = *(float2*)(D + X2 + 2);//ih = 2
	...
	for (int oic = 1; oic < (IC >> 3); ++oic) 
	{
		...
		*(float4*)(&Gs[ty][tg + 3][toc]) = g[3];
		...
		Winograd2D_f22x33_transform_X((float*)x);
		Ds[ty][ 0][tx] = x[0].x; Ds[ty][ 1][tx] = x[0].y;
		Ds[ty][ 2][tx] = x[1].x; Ds[ty][ 3][tx] = x[1].y;
		Ds[ty][ 4][tx] = x[2].x; Ds[ty][ 5][tx] = x[2].y;
		Ds[ty][ 6][tx] = x[3].x; Ds[ty][ 7][tx] = x[3].y;
		Ds[ty][ 8][tx] = x[4].x; Ds[ty][ 9][tx] = x[4].y;
		Ds[ty][10][tx] = x[5].x; Ds[ty][11][tx] = x[5].y;
		Ds[ty][12][tx] = x[6].x; Ds[ty][13][tx] = x[6].y;
		Ds[ty][14][tx] = x[7].x; Ds[ty][15][tx] = x[7].y;
		__syncthreads();
...
#pragma unroll
		...
		//load 1 group from X[N, IH, IW, IC]
		const int X0 = doffset + ic * GM;//[ic, n, ih, iw]
		const int X1 = X0 + DW, X2 = X0 + (DW << 1), X3 = X0 + DW * 3;
		x[0] = *(float2*)(D + X0); x[1] = *(float2*)(D + X0 + 2);//ih = 0
		x[2] = *(float2*)(D + X1); x[3] = *(float2*)(D + X1 + 2);//ih = 1
		x[4] = *(float2*)(D + X2); x[5] = *(float2*)(D + X2 + 2);//ih = 2
...
//[Standard:] Size = 9, Time = 1.23147 msec, Performace = 15694.5 GFlop/s
#define C3D_WG2D_KERNEL_C3
#ifndef C3D_WG2D_KERNEL_C3 
#define C3D_WG2D_KERNEL_C3

#define c3d_wg2d_c3(stream, oc_index, j_index, D, DH, DW, G, Y, OH, OW, N, IC, OC)\
	c3d_wg2d_kernel_C3\
		<<< dim3((GN>>5), (GM>>5)), dim3(32, 8), 0, stream >>>\
			(D, (DH*DW), DW, G, Y, (OH*OW), OW, (N*DH*DW), IC, OC, oc_index, j_index)\
...
//Size = 9, Time = 1.00934 msec, Performace = 19148.6 GFlop/s
//template<int OC = 128>  
__global__ void c3d_wg2d_kernel_C3(
	const float* __restrict__ D, int DH_DW, int DW,
	const float* __restrict__ G,
	      float* __restrict__ Y, int OH_OW, int OW,
